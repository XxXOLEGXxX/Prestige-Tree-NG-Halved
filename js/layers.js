...
addLayer("p", { //HALVEDLOL
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses //HALVEDLOL
			...
			if (player.shenanigans.unlocked) mult = mult.times(tmp.shenanigans.buyables[11].effect)
            return mult
        },
		...
		update(diff){
			player.p.time = player.p.time.plus(diff);
		},
...
addLayer("b", { //HALVEDLOL
		...
		addToBase() { //HALVEDLOL
			...
			if (hasUpgrade("paradox", 15)) base = base.plus(upgradeEffect("paradox", 15));
			return base;
		},
		effectBase() { //HALVEDLOL
			...
			// MULTIPLY
			if (player.shenanigans.unlocked) base = base.times(tmp.shenanigans.buyables[21].effect)
			if (hasMilestone("omega", 1)) base = base.times(tmp.omega.milestones[1].effect.b)
...
addLayer("g", { //HALVEDLOL
		...
		effBase() { //HALVEDLOL
			...
			// MULTIPLY
			if (hasMilestone("omega", 1)) base = base.times(tmp.omega.milestones[1].effect.g)
			if (player.shenanigans.unlocked) base = base.times(tmp.shenanigans.buyables[22].effect)
			if (hasAchievement("ng/", 15)) base = base.times(10/6)
			if (hasUpgrade("q", 12)) base = base.times(upgradeEffect("q", 12));
			if (inChallenge("h", 12)) base = base.div(tmp.h.baseDiv12)
		...
		effect() { //HALVEDLOL
			...
			if (hasUpgrade("paradox", 22)) eff = eff.times(upgradeEffect("paradox", 22))
			if (hasAchievement("ng/", 26)) eff = eff.times(1e20)
			if (hasUpgrade("paradox", 23)) eff = eff.pow(1.5)
			return eff;
		},
		effectDescription() {
		...
		powerExp() { //HALVEDLOL
			...
			if (hasAchievement("ng/", 33)) exp = exp.times(1.19)
		...
		upgrades: {
			...
			35: { //HALVEDLOL
				...
				description: "Nebula Energy, Honour, and Hyperspace Energy gains are boosted by Generator Power. Reward: Check Paradox layer.",
...
addLayer("t", { //HALVEDLOL
		...
		effBaseMult() { //HALVEDLOL
			let mult = new Decimal(1);
			if (hasAchievement("ng/", 24)) mult = mult.times(1.69)
			...
			if (hasMilestone("omega", 2)) mult = Decimal.pow(mult, tmp.omega.effect);
			return mult;
		},
		...
		tabFormat: ["main-display",
			...
			"blank",
			["display-text",
				function() {return 'You have ' + format(player.t.energy) + ' Time Energy, which boosts Point & Prestige Point gain'+(hasUpgrade("paradox", 14)?" and weakens PPP's softcap":"")+' by '+format(tmp.t.enEff)+'x'+(tmp.nerdMode?" ((x+1)^"+format(1.2*(hasUpgrade("t", 14)?1.3:1)*(hasUpgrade("q", 24)?7.5:1))+")":"")+(hasUpgrade("t", 24)?(", and provides "+formatWhole(tmp.t.enEff2)+" free Extra Time Capsules ("+(tmp.nerdMode?"log(x+1)^0.556":("next at "+format(tmp.t.nextEnEff2)))+")."):"")},
					{}],
			"blank",
			["display-text",
		...
		freeExtraTimeCapsules() {
			...
			free = free.times(tmp.shenanigans.buyables[32].effect)
			return free;
		},
		buyables: {
			rows: 1,
			...
			11: {
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    Amount: " + formatWhole(player[this.layer].buyables[this.id])+(e.gt(0)?(" + "+formatWhole(e)):"")+(inChallenge("h", 31)?("\nPurchases Left: "+String(5-player.h.chall31bought)):"")
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
                    return player.b.points.gte(tmp[this.layer].buyables[this.id].cost) && (inChallenge("h", 31) ? player.h.chall31bought<5 : true)},
                buy() { 
...
addLayer("e", { //HALVEDLOL
		...
		freeEnh() { //HALVEDLOL
			...
			enh = enh.times(tmp.shenanigans.buyables[33].effect)
			return enh;
		},
		...
		buyables: {
			...
			11: { //HALVEDLOL
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					...
					if (hasMilestone("omega", 2)) eff.first = Decimal.pow(eff.first, tmp.omega.effect);
					...
					if (hasMilestone("omega", 2)) eff.second = Decimal.pow(eff.second, tmp.omega.effect);
                    return eff;
                },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                ...
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
                    return player[this.layer].points.gte(tmp[this.layer].buyables[this.id].cost) && (inChallenge("h", 31) ? player.h.chall31bought<5 : true)},
                buy() { 
                    cost = tmp[this.layer].buyables[this.id].cost
...
addLayer("s", { //HALVEDLOL
        ...
        color: "#dfdfdf",
		effect() { return hasUpgrade("paradox", 13)?player.s.points.times(hasAchievement("ng/", 24)?1.69:1).pow(hasMilestone("omega", 2)?tmp.omega.effect:1).pow(1.81).div(3661).pow(6):new Decimal(1) },
		effectDescription() { return hasUpgrade("paradox", 13)?"which are increasing Dissolved Drained Distortions's effect by +"+format(this.effect().mul(100))+"% "+(tmp.nerdMode?" ((n^1.81/3661)^6*100)":""):"" },
        requires() { return new Decimal(1e120).times(Decimal.pow("1e180", Decimal.pow(player[this.layer].unlockOrder, 1.415038))) }, // Can be a function that takes requirement increases into account
		...
		space() { //HALVEDLOL
			...
			space = space.times(tmp.shenanigans.buyables[34].effect)
		...
		freeSpaceBuildings() {
			...
			x = x.times(tmp.shenanigans.buyables[34].effect)
			return x;
		},
		...
		buyables: {
			...
			11: { //HALVEDLOL
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let eff = Decimal.pow(x.div(2).plus(1).plus(tmp.s.freeSpaceBuildings).times(tmp.s.buildingPower), player.s.points.div(2).sqrt()).times(x.div(2).plus(tmp.s.buyables[this.id].freeLevels).times(tmp.s.buildingPower).max(1).times(4)).max(1);
				...
				display() { // Everything else displayed in the buyable button after the title
                   ...
                   "+(tmp.nerdMode?("Formula: level^sqrt(spaceEnergy/2)*level*2"):(" Space Energy boosts Point gain & Prestige Point gain by " + format(data.effect) +"x"))
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
			...
			12: { //HALVEDLOL
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal //HALVEDLOL
					let eff = x.plus(tmp.s.buyables[this.id].freeLevels).times(tmp.s.buildingPower).div(4).sqrt();
				...
				display() { // Everything else displayed in the buyable button after the title //HALVEDLOL
                    ...
                    return (tmp.nerdMode?("Cost Formula: "+format(tmp.s.buildingBaseCosts[this.id])+"^((x*"+format(tmp.s.buildScalePower)+")^"+format(tmp[this.layer].buyables[this.id].costExp)+")*"+format(tmp.s.buildingBaseCosts[this.id])+"/"+format(tmp.s.divBuildCosts)):("Cost: " + formatWhole(data.cost) + " Generator Power"))+"\n\
                    Level: " + formatWhole(player[this.layer].buyables[this.id])+(data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
                    "+(tmp.nerdMode?("Formula: sqrt(level/4)"):("Adds to base of Booster/Generator effects by +" + format(data.effect)))
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
                    return player.g.power.gte(tmp[this.layer].buyables[this.id].cost) && layers.s.space().gt(0)},
			...
			14: { //HALVEDLOL
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal //HALVEDLOL
					let ret = x.div(2).plus(tmp.s.buyables[this.id].freeLevels).times(tmp.s.buildingPower).times((hasUpgrade("s", 15))?2:1).add(1).pow(1.125); //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    Level: " + formatWhole(player[this.layer].buyables[this.id])+(data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
					"+(tmp.nerdMode?("Formula: "+(data.effect.gte(1e6)?("log(level/2"+(extForm==1?"":"*2")+"+1)*2.08e5"):("(level"+(extForm==1?"":"*2")+"+1)^1.125"))):("<b>Discount One</b> is raised to the power of " + format(data.effect)))
                },
			...
			15: { //HALVEDLOL
				...
				freeLevels() {
					...
					if (hasMilestone("omega", 2)) levels = levels.plus(125)
					return levels;
				},
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let ret = x.div(2).plus(tmp.s.buyables[this.id].freeLevels).times(tmp.s.buildingPower).div(2);
					if (hasUpgrade("q", 32)) ret = ret.times(1.5); //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    Level: " + formatWhole(player[this.layer].buyables[this.id])+(data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
					"+(tmp.nerdMode?("Formula: level/2"+(hasUpgrade("q", 32)?"":"/2")):("Add " + formatWhole(data.effect)+" levels to all previous Space Buildings."))
                },
			...
			16: { //HALVEDLOL
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let ret = x.div(2).plus(tmp.s.buyables[this.id].freeLevels).times(tmp.s.buildingPower).plus(1).sqrt();
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    Level: " + formatWhole(player[this.layer].buyables[this.id]) + (data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
					"+(tmp.nerdMode?("Formula: sqrt(level/2+1)"):("Multiply Damned Soul gain by " + format(data.effect)+"."))
                },
			...
			17: { //HALVEDLOL
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let ret = Decimal.pow("1e10000", x.div(2).plus(tmp.s.buyables[this.id].freeLevels).times(tmp.s.buildingPower).pow(1.1));
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    return (tmp.nerdMode?("Cost Formula: "+format(tmp.s.buildingBaseCosts[this.id])+"^((x*"+format(tmp.s.buildScalePower)+")^"+format(data.costExp)+")*"+format(tmp.s.buildingBaseCosts[this.id])+"/"+format(tmp.s.divBuildCosts)):("Cost: " + formatWhole(data.cost) + " Generator Power"))+"\n\
                    Level: " + formatWhole(player[this.layer].buyables[this.id]) + (data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
					"+(tmp.nerdMode?("Formula: 1e10,000^((level/2)^1.1)"):("Divide the requirement of Phantom Souls by " + format(data.effect)+"."))
                },
			...
			18: { //HALVEDLOL
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let ret = x.plus(tmp.s.buyables[this.id].freeLevels).times(tmp.s.buildingPower).div(3)
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    return (tmp.nerdMode?("Cost Formula: "+format(tmp.s.buildingBaseCosts[this.id])+"^((x*"+format(tmp.s.buildScalePower)+")^"+format(data.costExp)+")*"+format(tmp.s.buildingBaseCosts[this.id])+"/"+format(tmp.s.divBuildCosts)):("Cost: " + formatWhole(data.cost) + " Generator Power"))+"\n\
                    Level: " + formatWhole(player[this.layer].buyables[this.id]) + (data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
					"+(tmp.nerdMode?("Formula: level/3"):("Get " + format(data.effect)+" more Free Quirk Layers."))
                },
			...
			19: { //HALVEDLOL
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let ret = x.plus(tmp.s.buyables[this.id].freeLevels).times(tmp.s.buildingPower).div(2e3).plus(1)
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    return (tmp.nerdMode?("Cost Formula: "+format(tmp.s.buildingBaseCosts[this.id])+"^((x*"+format(tmp.s.buildScalePower)+")^"+format(data.costExp)+")*"+format(tmp.s.buildingBaseCosts[this.id])+"/"+format(tmp.s.divBuildCosts)):("Cost: " + formatWhole(data.cost) + " Generator Power"))+"\n\
                    Level: " + formatWhole(player[this.layer].buyables[this.id]) + (data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
					"+(tmp.nerdMode?("Formula: level/2,000+1"):("The Hyperspace Energy gain exponent is multiplied by " + format(data.effect)+"."))
                },
			...
			20: { //HALVEDLOL
				...
				effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a decimal
					let ret = x.plus(tmp.s.buyables[this.id].freeLevels).times(tmp.s.buildingPower).div(500)
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    return (tmp.nerdMode?("Cost Formula: "+format(tmp.s.buildingBaseCosts[this.id])+"^((x*"+format(tmp.s.buildScalePower)+")^"+format(data.costExp)+")*"+format(tmp.s.buildingBaseCosts[this.id])+"/"+format(tmp.s.divBuildCosts)):("Cost: " + formatWhole(data.cost) + " Generator Power"))+"\n\
                    Level: " + formatWhole(player[this.layer].buyables[this.id]) + (data.freeLevels.gt(0)?(" + "+formatWhole(data.freeLevels)):"") + "\n\
					"+(tmp.nerdMode?("Formula: (level/5)%"):("Hyper Building Power is increased by " + format(data.effect.times(100))+"%."))
                },
...
addLayer("sb", { //HALVEDLOL
		...
		gainMult() {  //HALVEDLOL
			let mult = new Decimal(1);
			if (hasUpgrade("ss", 21)) mult = mult.div(1.1); //HALVEDLOL
		...
		effectBase() { //HALVEDLOL
			let base = new Decimal(3); //HALVEDLOL
			...
			if (hasUpgrade("paradox", 21)) base = base.times(upgradeEffect("paradox", 21))
			...
			base = base.times(tmp.shenanigans.buyables[31].effect)
			return base
		},
		effect() { //HALVEDLOL
			if (!unl(this.layer)) return new Decimal(1);
		...
		spectralEach() {
			...
			return player.sb.points.div(2);
		},
		spectralTotal() {
			return tmp.sb.spectralEach.times(player.sb.points.div(2));
		},
		tabFormat: ["main-display",
			"prestige-button",
			"blank",
...
addLayer("sg", { //HALVEDLOL
		...
		effectBase() { //HALVEDLOL
			...
			base = base.times(tmp.shenanigans.buyables[35].effect)
			return base;
		},
		effect() { //HALVEDLOL
			if (!unl(this.layer)) return new Decimal(0);
		...
		spectralTotal() { //HALVEDLOL
			if (!((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("sg"):false)) return new Decimal(0);
			return player.sg.time.div(2).plus(1).log10().times(player.sg.points.div(2).pow(1.5)).pow(.95).times(1.1).floor();
		},
		tabFormat: ["main-display",
			"prestige-button",
			"blank",
...
addLayer("h", { //HALVEDLOL
		...
		passiveGeneration() { return (hasMilestone("m", 2)&&player.ma.current!="h")?0.5:0 }, //HALVEDLOL
		milestones: {
			0: {
		...
		challenges: { //HALVEDLOL //HALVEDLOL
			rows: 4,
			cols: 2,
			11: { //HALVEDLOL
				name: "Upgrade Desert",
				...
				rewardDescription: "Unlock Quirk Upgrades, and the Hindrance Spirit effect is raised to the power of 1.1.",
			...
			31: { //HALVEDLOL
				...
				challengeDescription() {
					...
					return "You can only buy 5 Enhancers & Extra Time Capsules (total), Enhancer/Extra Time Capsule automation is disabled, and Point generation is brought to the "+format(tmp.h.pointRoot31)+"th root<br>Completions: "+formatWhole(challengeCompletions("h", 31))+(infLim?"":("/"+lim));
				},
				...
				rewardEffect() { 
					let eff = Decimal.div(2.5, Decimal.add((hasUpgrade("ss", 33)?(player.timePlayed||0):player.q.time.div(4)), 1).cbrt().pow(hasUpgrade("ss", 23)?(-1):1)).plus(1).pow(challengeCompletions("h", 31)/2).times(tmp.n.realDustEffs2?tmp.n.realDustEffs2.blueOrange:new Decimal(1)).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?3:1);
					if (!eff.eq(eff)) eff = new Decimal(1);
					return eff;
				},
				rewardDisplay() { return format(this.rewardEffect())+"x" },
				formula() { return "(2.5"+(hasUpgrade("ss", 23)?"*":"/")+"cbrt(time/4+1)+1)^(completions/2)" },
			},
			32: { //HALVEDLOL
				name: "Option D",
				scalePower() {
				...
				rewardEffect() { 
					let eff = softcap("option_d", Decimal.pow(50, Decimal.pow(challengeCompletions("h", 32)/2, 1.5))).times(tmp.n.realDustEffs2?tmp.n.realDustEffs2.blueOrange:new Decimal(1));
					if (!eff.eq(eff)) eff = new Decimal(1);
					return eff;
				},
				rewardDisplay() { return format(tmp.h.challenges[32].rewardEffect)+"x" },
				formula: "50^((completions/2)^1.5)",
...
addLayer("q", { //HALVEDLOL
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?.008:.00775) }, // Prestige currency exponent //HALVEDLOL
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
		...
		enEff() { //HALVEDLOL
			if (!unl(this.layer)) return new Decimal(1);
			let eff = player.q.energy.div(2).plus(1).pow(1.5);
			if (hasUpgrade("q", 23)) eff = eff.pow(2);
		...
		passiveGeneration() { return (hasMilestone("ba", 0)&&player.ma.current!="q")?0.5:0 },
		tabFormat: { //HALVEDLOL
			"Main Tab": {
				content: [
					"main-display",
					...
					"blank",
					["display-text",
						function() {return 'You have ' + formatWhole(player.q.energy)+' Quirk Energy ('+(tmp.nerdMode?('Base Gain: (timeInRun^(quirkLayers/2-1))'):'generated by Quirk Layers')+'), which multiplies Point and Generator Power gain by ' + format(tmp.q.enEff)+(tmp.nerdMode?(" ((x/2+1)^"+format(hasUpgrade("q", 23)?2:1.5)+"*"+format(improvementEffect("q", 23))+")"):"")},
							{}],
					"blank",
					"milestones", "blank",
					"blank",
			...
			Improvements: {
				...
				content: [
					...
					"blank",
					["display-text",
						function() {return 'You have ' + formatWhole(player.q.energy)+' Quirk Energy ('+(tmp.nerdMode?('Base Gain: (timeInRun^(quirkLayers/2-1))'):'generated by Quirk Layers')+'), which has provided the below Quirk Improvements (next at '+format(tmp.q.impr.overallNextImpr)+')'},
							{}],
					"blank",
					"improvements"],
			},
		},
		freeLayers() { //HALVEDLOL
			let l = new Decimal(0);
		...
		buyables: { //HALVEDLOL
			...
			11: {
				...
				costBase() {
					...
					if (hasUpgrade("q", 43)) base = base.sub(.125); //HALVEDLOL
		...
		upgrades: { //HALVEDLOL
			...
			11: { //HALVEDLOL
				...
				effect() { return player.q.total.div(2).plus(1).log10().plus(1).pow(player.q.upgrades.length/2).pow(improvementEffect("q", 11)) },
				effectDisplay() { return format(tmp.q.upgrades[11].effect)+"x" },
				formula: "(log(quirks/2+1)+1)^(upgrades/2)",
			},
			12: { //HALVEDLOL
				title: "Back To Row 2",
				...
				effect() { return player.q.total.div(2).plus(1).log10().plus(1).pow(1.125).times(improvementEffect("q", 12)) },
				effectDisplay() { return format(tmp.q.upgrades[12].effect)+"x" },
				formula: "(log(x/2+1)+1)^1.125",
			},
			13: { //HALVEDLOL
			...
			14: { //HALVEDLOL
				...
				effect() { 
					...
					q = softcap("q14_q", q);
					return {
						h: q.div(2).plus(1).cbrt().pow(improvementEffect("q", 13)),
						q: h.div(2).plus(1).root(4).pow(improvementEffect("q", 13)),
					};
				},
			...
			15: { //HALVEDLOL
				...
				unlocked() { return player[this.layer].pseudoUpgs.includes(Number(this.id)) },
				effect() { return player.q.points.div(2).plus(1) },
			...
			21: { //HALVEDLOL
				...
				effect() { return Decimal.pow(1.125, player.sb.points.div(2)).pow(improvementEffect("q", 21)) },
			...
			22: { //HALVEDLOL
				...
				effect() { return player.q.total.div(2).plus(1).log10().sqrt().times(improvementEffect("q", 22)).floor() },
				effectDisplay() { return "+"+formatWhole(tmp.q.upgrades[22].effect) },
				formula: "floor(sqrt(log(x/2+1)))",
			},
			23: { //HALVEDLOL
			...
			25: { //HALVEDLOL
				...
				unlocked() { return player[this.layer].pseudoUpgs.includes(Number(this.id)) },
				effect() { return player.i.nb.div(2).plus(1).log10().plus(1).pow(2) },
				effectDisplay() { return "brought to the "+format(upgradeEffect("q", 25))+"th root" },
				formula: "(log(x/2+1)+1)^2",
			},
			31: { //HALVEDLOL
				...
				effect() { return player.q.buyables[11].div(2).sqrt().times(0.4).times(improvementEffect("q", 31)) },
				effectDisplay() { return "+"+format(tmp.q.upgrades[31].effect) },
				formula: "sqrt(x/2)*0.4",
			},
			32: { //HALVEDLOL
			...
			34: { //HALVEDLOL
				...
				effect() { return tmp.b.addToBase.div(2).plus(1).root(2.5).times(improvementEffect("q", 32)) },
			...
			43: { //HALVEDLOL
				title: "More Layers",
				description: "Quirk Layers cost scale 12.5% slower.",
			...
			45: { //HALVEDLOL
				...
				description: "The Hindrance Spirit effect is raised to the power of 50 (after softcaps), and gain 100x more Nebula Energy.",
		...
		impr: { //HALVEDLOL
			...
			11: { //HALVEDLOL
				...
				unlocked() { return hasUpgrade("q", 41) },
				effect() { return Decimal.mul(0.05, getImprovements("q", 11).plus(tmp.q.impr.free).div(2)).plus(1) },
			...
			12: { //HALVEDLOL
				...
				unlocked() { return hasUpgrade("q", 41) },
				effect() { return Decimal.mul(0.025, getImprovements("q", 12).plus(tmp.q.impr.free).div(2)).plus(1) },
			...
			13: { //HALVEDLOL
				...
				unlocked() { return hasUpgrade("q", 41) },
				effect() { return Decimal.mul(0.125, getImprovements("q", 13).plus(tmp.q.impr.free).div(2)).plus(1) },
			...
			21: { //HALVEDLOL
				...
				unlocked() { return hasUpgrade("q", 42) },
				effect() { return Decimal.mul(0.75, getImprovements("q", 21).plus(tmp.q.impr.free).div(2)).plus(1) },
			...
			22: { //HALVEDLOL
				...
				unlocked() { return hasUpgrade("q", 42) },
				effect() { return Decimal.mul(0.1, getImprovements("q", 22).plus(tmp.q.impr.free).div(2)).plus(1) },
			...
			23: { //HALVEDLOL
				...
				unlocked() { return hasUpgrade("q", 42) },
				effect() { return Decimal.pow(5e12, Decimal.pow(getImprovements("q", 23).plus(tmp.q.impr.free).div(2), 1.25)) },
				effectDisplay() { return format(tmp.q.impr[23].effect)+"x" },
				formula: "5e12^((x/2)^1.25)",
			},
			31: { //HALVEDLOL
				num: 7,
				...
				effect() { return Decimal.mul(0.25, getImprovements("q", 31).plus(tmp.q.impr.free).div(2)).plus(1) },
			...
			32: { //HALVEDLOL
				...
				unlocked() { return hasUpgrade("q", 44) },
				effect() { return Decimal.mul(0.1, getImprovements("q", 32).plus(tmp.q.impr.free).div(2)).plus(1) },
			...
			33: { //HALVEDLOL
				...
				unlocked() { return hasUpgrade("q", 44) },
				effect() { return Decimal.pow(1e4, Decimal.pow(getImprovements("q", 33).plus(tmp.q.impr.free).div(2), 1.1)) },
			...
			41: { //HALVEDLOL
				...
				effect() { return Decimal.pow("1e200", Decimal.pow(getImprovements("q", 41).plus(tmp.q.impr.free).div(2), 0.9)) },
				effectDisplay() { return format(tmp.q.impr[41].effect)+"x" },
				formula: "1e200^((x/2)^0.9)",
			},
			42: { //HALVEDLOL
				num: 281,
				...
				effect() { return Decimal.pow(5, Decimal.pow(getImprovements("q", 42).plus(tmp.q.impr.free).div(2), 0.75)) },
			...
			43: { //HALVEDLOL
				...
				effect() { return Decimal.mul(Decimal.pow(getImprovements("q", 43).plus(tmp.q.impr.free).div(2), 0.8), 1.125) },
				effectDisplay() { return "+"+format(tmp.q.impr[43].effect) },
				formula: "1.125*((x/2)^0.8)",
			},
		},
})
...
addLayer("o", { //HALVEDLOL
	name: "solarity", // This is optional, only used in a few places, If absent it just uses the layer id.
		...
		passiveGeneration() { return player.ma.current=="o"?0:(hasMilestone("m", 0)?0.5:(hasMilestone("o", 0)?0.025:0)) }, //HALVEDLOL
		solPow() { //HALVEDLOL
			let pow = new Decimal(1);
...
addLayer("ss", { //HALVEDLOL
		...
		upgrades: { //HALVEDLOL
			...
			21: { //HALVEDLOL
				...
				description: "Super Boosters & Super Generators are 10% cheaper.",
			...
			32: { //HALVEDLOL
				...
				effect() { return player.q.buyables[11].div(2).sqrt().div(1.25) },
				effectDisplay() { return "+"+format(tmp.ss.upgrades[32].effect) },
				formula: "sqrt(x/2)/1.25",
			},
			33: { //HALVEDLOL
				...
				effect() { return player.o.buyables[11].div(2).plus(1).log10().div(20) },
...
addLayer("m", { //HALVEDLOL
		...
		passiveGeneration() { return (hasMilestone("hn", 1)&&player.ma.current!="m")?0.5:0 }, //HALVEDLOL
        layerShown(){return player.h.unlocked&&player.o.unlocked },
        branches: ["o","h","q"],
		spellTime() {  //HALVEDLOL
			let time = new Decimal(30);
			if (hasMilestone("m", 3)) time = time.times(tmp.m.spellInputAmt.div(200).plus(1).log10().plus(1));
			return time;
		},
		spellPower() {  //HALVEDLOL
			if (!unl(this.layer)) return new Decimal(0);
			let power = new Decimal(1);
		...
		mainHexEff() { return player.m.hexes.div(2).times(1.5).plus(1).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?2.5:3.75) },  //HALVEDLOL
		hexEff() { return softcap("hex", tmp.m.mainHexEff) },
		update(diff) { //i'd rather not touch that for now
		...
		hexEffDesc() { //HALVEDLOL
			let nerd = (tmp.nerdMode?" (x+1)^2.5":"")
		...
		castAllSpells(noSpend=false, diff=1) { //HALVEDLOL
		...
		buyables: { //HALVEDLOL
			...
			11: { //HALVEDLOL
				...
				effect() {
					let power = tmp.m.spellPower.times(player.m.spellInputs[this.id].div(2).max(1).log10().plus(1));
					if (player.m.spellTimes[this.id].eq(0)) power = new Decimal(0);
					let eff = power.div(2).plus(1)
					if (hasUpgrade("ba", 31)) eff = Decimal.pow(1.05, power).times(eff); //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
					...
					if (hasMilestone("m", 3)) display += "\n "+(tmp.nerdMode?("Formula: ((log(inserted/2+1)+1)/2+1)/1.5"):("To Insert: "+formatWhole(tmp.m.spellInputAmt.div((player.m.distrAll && hasMilestone("m", 4))?tmp.m.spellsUnlocked:1))));
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
			...
			12: { //HALVEDLOL
				...
				effect() { //HALVEDLOL
					...
					if (hasUpgrade("ba", 31)) eff = Decimal.pow(1.05, power).times(eff);
				...
				display() { // Everything else displayed in the buyable button after the title
					...
					Time: "+formatTime(player.m.spellTimes[this.id]||0);
					if (hasMilestone("m", 3)) display += "\n "+(tmp.nerdMode?("Formula: ((log(inserted/2+1)+1)/5+1)/1.2"):("To Insert: "+formatWhole(tmp.m.spellInputAmt.div((player.m.distrAll && hasMilestone("m", 4))?tmp.m.spellsUnlocked:1))));
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
			...
			13: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
					...
					Time: "+formatTime(player.m.spellTimes[this.id]||0);
					if (hasMilestone("m", 3)) display += "\n "+(tmp.nerdMode?("Formula: (log(inserted/2+1)+1)*1.125"):("To Insert: "+formatWhole(tmp.m.spellInputAmt.div((player.m.distrAll && hasMilestone("m", 4))?tmp.m.spellsUnlocked:1))));
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
			...
			14: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
					...
					Time: "+formatTime(player.m.spellTimes[this.id]||0);
					if (hasMilestone("m", 3)) display += "\n "+(tmp.nerdMode?("Formula: 1-1/sqrt(log(log(inserted/2+1)+1)/500+1)"):("To Insert: "+formatWhole(tmp.m.spellInputAmt.div((player.m.distrAll && hasMilestone("m", 4))?tmp.m.spellsUnlocked:1))));
					return display;
                },
			...
			15: { //HALVEDLOL
				...
				effect() {
					let power = tmp.m.spellPower.times(player.m.spellInputs[this.id].div(2).max(1).log10().plus(1));
					if (player.m.spellTimes[this.id].eq(0)) power = new Decimal(0);
					let eff = Decimal.sub(1, Decimal.div(1, power.plus(1).log10().times(70).plus(1).sqrt()));
					return eff;
				},
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
					...
					Time: "+formatTime(player.m.spellTimes[this.id]||0);
					if (hasMilestone("m", 3)) display += "\n "+(tmp.nerdMode?("Formula: 1-1/sqrt(log(log(inserted/2+1)+1)*70+1)"):("To Insert: "+formatWhole(tmp.m.spellInputAmt.div((player.m.distrAll && hasMilestone("m", 4))?tmp.m.spellsUnlocked:1))));
					return display;
                },
			...
			16: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
					...
					Time: "+formatTime(player.m.spellTimes[this.id]||0);
					if (hasMilestone("m", 3)) display += "\n "+(tmp.nerdMode?("Formula: (log(inserted/2+1)+1)^200"):("To Insert: "+formatWhole(tmp.m.spellInputAmt.div((player.m.distrAll && hasMilestone("m", 4))?tmp.m.spellsUnlocked:1))));
					return display;
                },
		...
		milestones: { //HALVEDLOL
			0: { //HALVEDLOL
				...
				effectDescription: "Automatically gain 50% of Solarity gain & Solarity buyables every second.",
			},
			1: {
			...
			2: { //HALVEDLOL
				...
				effectDescription: "Automatically gain 50% of Hindrance Spirit gain every second.",
			},
			3: {
...
addLayer("ba", { //HALVEDLOL
		...
		passiveGeneration() { return (hasMilestone("hn", 1)&&player.ma.current!="ba")?0.5:0 }, //HALVEDLOL
		dirBase() { return player.ba.points.times(5) }, //HALVEDLOL
		posGainMult() { //HALVEDLOL
		...
		posNerf() { return tmp.ba.noNerfs?new Decimal(1):(player.ba.pos.plus(1).sqrt().pow(inChallenge("h", 41)?100:1)) }, //HALVEDLOL
		negGainMult() { //HALVEDLOL
			let mult = new Decimal(0.5);
		...
		negNerf() { return tmp.ba.noNerfs?new Decimal(1):(player.ba.neg.plus(1).log10().plus(1).sqrt().pow(inChallenge("h", 41)?100:1).div(hasUpgrade("ba", 14)?2:1.5).max(1)) }, //HALVEDLOL
		tabFormat: ["main-display",
			"prestige-button",
			"resource-display",
			"blank",
			...
			["row", [
				["column", [["display-text", function() {return tmp.nerdMode?("Gain Formula: "+format(tmp.ba.dirBase)+"^(1-barPercent/100)*(1-barBercent/100)"+(tmp.ba.negGainMult.eq(1)?"":("*"+format(tmp.ba.negGainMult)))):("+"+format(tmp.ba.negGain)+"/sec")}, {}], ["display-text", function() {return "Negativity: "+format(player.ba.neg)}, {}], ["display-text", function() {return (tmp.nerdMode?("Buff Formula: "+((hasUpgrade("ba", 13))?"(x/2+1)^5":"x/2+1")):("Buff: Multiply each Quirk Layer by "+format(tmp.ba.negBuff)))}, {}], ["display-text", function() {return tmp.ba.noNerfs?"":(tmp.nerdMode?("Nerf Formula: "+(hasUpgrade("ba", 14)?"sqrt(log(x+1)+1)"+(inChallenge("h", 41)?"^100":"")+"/2":"sqrt(log(x+1)+1)")):("Nerf: Divide the Positivity buff by "+format(tmp.ba.negNerf)))}, {}], "blank", ["row", [["upgrade", 11], ["upgrade", 13]]]], {"max-width": "240px"}], 
				"blank", "blank", "blank", 
				["column", 
				[["display-text", function() {return tmp.nerdMode?("Gain Formula: "+format(tmp.ba.dirBase)+"^(barPercent/100)*(barBercent/100)"+(tmp.ba.posGainMult.eq(1)?"":("*"+format(tmp.ba.posGainMult)))):("+"+format(tmp.ba.posGain)+"/sec")}, {}], ["display-text", function() {return "Positivity: "+format(player.ba.pos)}, {}], ["display-text", function() {return (tmp.nerdMode?("Buff Formula: log(x/2+1)+1"):("Buff: Multiply the Subspace & Time base by "+format(tmp.ba.posBuff)))}, {}], ["display-text", function() {return tmp.ba.noNerfs?"":(tmp.nerdMode?("Nerf Formula: sqrt(x+1)"+(inChallenge("h", 41)?"^100":"")):("Nerf: Divide the Negativity buff by "+format(tmp.ba.posNerf)))}, {}], "blank", ["row", [["upgrade", 14], ["upgrade", 12]]]], {"max-width": "240px"}]], {"visibility": function() { return player.ba.unlocked?"visible":"hidden" }}],
		...
		upgrades: {
			...
			12: { //HALVEDLOL
				...
				currencyLayer: "ba",
				unlocked() { return hasMilestone("ba", 3) },
				effect() { return softcap("ba12", player.ba.pos.div(2).plus(1).log10().cbrt().div(20)) },
				effectDisplay() { return "+"+format(tmp.ba.upgrades[12].effect.times(100))+"%" },
				formula: "cbrt(log(x/2+1))*5",
			},
			13: { //HALVEDLOL
				title: "Negative Energy",
				description: "Raise the Negativity buff to the power of 5.",
			...
			14: { //HALVEDLOL
				title: "Positive Vibe",
				description: "Halve-halve the Negativity nerf.",
				cost() { return new Decimal(player.ma.current=="ba"?"1e189500":25e7) },
				currencyDisplayName: "positivity",
				currencyInternalName: "pos",
				currencyLayer: "ba",
			...
			21: { //HALVEDLOL
				title: "Neutral Atom",
				description: "The Hindrance Spirit effect is raised to the power of 4.5.",
				cost() { return new Decimal(player.ma.current=="ba"?"1e189500":25e7) },
			...
			32: { //HALVEDLOL
				...
				unlocked() { return hasChallenge("h", 41) },
				effect() { 
					let eff = softcap("ba32", player.ba.pos.div(2).plus(1).log10().div(50).plus(1).pow(5));
		...
		milestones: {
			0: { //HALVEDLOL
				...
				effectDescription: "Gain 50% of Quirks gained every second, and keep Quirk Upgrades on all resets.",
			},
			1: {
...
addLayer("ps", { //HALVEDLOL
		...
		update(diff) { //NO NO NO NO NO NO NO NO NO NO-
		...
		soulGain() { //HALVEDLOL
			let gain = (((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?Decimal.pow(tmp.ps.soulGainExp, player.ps.points.div(2)):Decimal.pow(player.ps.points.div(2), tmp.ps.soulGainExp)).div(9.4).times(layers.ps.soulGainMult());
			return gain;
		},
		gainDisplay() { //HALVEDLOL
		...
		soulEffExp() { //HALVEDLOL
			let exp = new Decimal(7.5e2);
		...
		soulEff() { //HALVEDLOL
			let eff = player.ps.souls.div(2).plus(1).pow(layers.ps.soulEffExp());
			return eff;
		},
		powerGain() { return player.ps.souls.div(2).plus(1).times(tmp.ps.buyables[21].effect).times(tmp.n.dustEffs.purple) }, //HALVEDLOL
		powerExp() { return player.ps.points.div(2).sqrt().times(tmp.ps.buyables[21].effect) }, //HALVEDLOL
		tabFormat: { //HALVEDLOL
			"Main Tab": {
				content: ["main-display",
					...
					"resource-display",
					"blank",
					["display-text", function() { return "You have "+formatWhole(player.ps.souls)+" Damned Souls "+(tmp.nerdMode?("(Formula: ("+(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("ps"):false)?(format(tmp.ps.soulGainExp)+"^PS"):("PS^"+format(tmp.ps.soulGainExp)))+")*"+format(tmp.ps.soulGainMult.div(10))+")"):("(Gain: "+tmp.ps.gainDisplay+")"))+": Divide Quirk Improvement requirements by "+format(tmp.ps.soulEff)+(tmp.nerdMode?(" (x/2+1)^("+formatWhole(tmp.ps.soulEffExp)+")"):"") }],
					"blank",
					["buyable", 11],
				],
			},
			Boosters: {
				...
				content: [
					...
					"blank",
					["display-text",
						function() {return 'You have ' + formatWhole(player.ps.power)+' Phantom Power'+(tmp.nerdMode?(" (Gain Formula: (damned/2+1), Exp Formula: sqrt(ps/2))"):" (+"+format(tmp.ps.powerGain)+"/sec (based on Damned Souls), then raised to the power of "+format(tmp.ps.powerExp)+" (based on Phantom Souls))")+', which has provided the below Phantom Boosters (next at '+format(tmp.ps.impr.overallNextImpr)+')'},
							{}],
					"blank",
					"improvements"],
			},
		...
		buyables: { //HALVEDLOL
			...
			11: { //HALVEDLOL
				...
				effects(adj=0) {
					...
					if (x.gte(2)) data.damned = x.div(2).sub(1).times(0.5).div(10/9.4).plus(1); //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
					...
					if (Object.keys(next).length>0) {
						...
						if (next.damned) {
							display += "\n"
							if (curr.damned) display += "Multiply Damned Soul gain & effect exponent by "+format(curr.damned)+(tmp.nerdMode?" ((x/2-1)*0.5+1)":""); //HALVEDLOL
			...
			21: { //HALVEDLOL
				...
				effect() { //HALVEDLOL
					return player[this.layer].buyables[this.id].div(50).plus(1).pow(1.5);
				},
				effect2() { //HALVEDLOL
					return player[this.layer].buyables[this.id].div(20).plus(1);
				},
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    let display = ((tmp.nerdMode?("Cost Formula: (10^(2^x))*1e22"):("Cost: " + formatWhole(data.cost) + " Phantom Power"))+"\n\
                    Amount: " + formatWhole(player[this.layer].buyables[this.id])+"\n\
					Effect: "+(tmp.nerdMode?("Formula 1: (x/50+1)^1.5, Formula 2: (x/20+1)"):("Multiply Phantom Power gain/exponent by "+format(tmp.ps.buyables[this.id].effect)+", and boost Phantom Booster effectiveness by "+format(tmp.ps.buyables[this.id].effect2.sub(1).times(100))+"%")))
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
		...
		impr: {
			...
			power() { return tmp.ps.buyables[21].effect2.div(2).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.05:1) }, //HALVEDLOL
			...
			11: { //HALVEDLOL
				...
				unlocked() { return hasMilestone("hn", 7) },
				effect() { return getImprovements("ps", 11).div(2).times(tmp.ps.impr.power).div(40).plus(1).sqrt() },
				effectDisplay() { return "+"+format(tmp.ps.impr[11].effect.sub(1).times(100))+"% (multiplicative)" },
				formula: "sqrt(x/2*2.5%)",
				style: {height: "150px", width: "150px"},
			},
			12: { //HALVEDLOL
				num: 2,
				...
				unlocked() { return hasMilestone("hn", 7) },
				effect() { return Decimal.pow(5, getImprovements("ps", 11).div(2).times(tmp.ps.impr.power).pow(1.75)) },
			...
			21: { //HALVEDLOL
				...
				unlocked() { return hasMilestone("hn", 7) },
				effect() { return getImprovements("ps", 21).times(tmp.ps.impr.power).div(40).plus(1) },
			...
			22: { //HALVEDLOL
				...
				unlocked() { return hasMilestone("hn", 7) },
				effect() { return getImprovements("ps", 22).times(tmp.ps.impr.power).div(40).plus(1) },
			...
			31: { //HALVEDLOL
				...
				effect() { return getImprovements("ps", 31).times(tmp.ps.impr.power).div(2).plus(1).log10().div(50).plus(1) },
				effectDisplay() { return format(Decimal.sub(1, tmp.ps.impr[31].effect.pow(-1)).times(100))+"% slower" },
				formula: "log(x/2+1)/50+1",
				style: {height: "150px", width: "150px"},
			},
			32: { //HALVEDLOL
				num: 1751,
				...
				effect() { return getImprovements("ps", 31).times(tmp.ps.impr.power).div(2).pow(1.5).times(player.ps.points.div(2)).plus(1).log10().plus(1).pow(1.1) },
				effectDisplay() { return "brought to the "+format(tmp.ps.impr[32].effect)+"th root" },
				formula: "(log(((x/2)^1.5)*(PS/2)+1)+1)^1.1",
				style: {height: "150px", width: "150px"},
			},
		},
})

addLayer("hn", { //HALVEDLOL
		...
		exp() { return {m: new Decimal(0.025), ba: new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?0.035:0.02)} }, //HALVEDLOL
		...
		passiveGeneration() { return (hasMilestone("ma", 1)&&player.ma.current!="hn")?0.5:0 }, //HALVEDLOL
		canReset() {
		...
		milestones: {
			...
			1: {
				...
				effectDescription: "Gain 50% of Magic & Balance Energy every second.",
			},
			2: {
		...
		upgrades: {
			...
			12: { //HALVEDLOL
				...
				effect() { return softcap("hn12", player.hn.total.div(2).plus(1).pow(1e2)) },
			...
			13: { //HALVEDLOL
				...
				effect() { return tmp.p.upgrades[13].effect.div(2).max(1).log10().plus(1).log10().times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("hn"):false)?40:20).plus(1) },
			...
			21: { //HALVEDLOL
				...
				cap() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("hn"):false)?.455:.45) },
				effect() { return player.m.hexes.div(2).plus(1).log10().plus(1).log10().times(0.075).min(tmp.hn.upgrades[this.id].cap) },
				...
				formula() { return "log(log(x/2+1)+1)*7.5, maxed at "+format(tmp.hn.upgrades[this.id].cap.times(100))+"%" },
			},
			22: { //HALVEDLOL
				...
				effect() { return Decimal.pow(5, player.ps.souls.div(2).plus(1).log10().plus(1).log10().sqrt().times(3)).times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("hn"):false)?1.5:1) },
			...
			25: { //HALVEDLOL
				...
				effect() { return player.hs.points.div(2).times(player.n.points.div(2).pow(2)).plus(1).log10().plus(1).log10().plus(1) },
				effectDisplay() { return format(tmp.hn.upgrades[25].effect)+"x" },
				formula: "log(log(HS/2*((N/2)^2)+1)+1)+1",
				style: {"font-size": "9px"},
			},
			31: { //HALVEDLOL
			...
			35: { //HALVEDLOL
				...
				unlocked() { return hasUpgrade("hn", 53) && hasUpgrade("hn", 54) && player.n.unlocked },
				effect() { return player.n.purpleDust.div(2).times(player.n.blueDust.div(2)).plus(1).pow(5) },
			...
			41: { //HALVEDLOL
				...
				effect() { return player.ps.power.div(2).plus(1).log10().plus(1).log10().times(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("hn"):false)?1.57:1.35).plus(1) },
				effectDisplay() { return "^"+format(tmp.hn.upgrades[41].effect) },
				formula: "log(log(x/2+1)+1)*1.35+1",
				style: {"font-size": "9px"},
			},
			42: { //HALVEDLOL
			...
			43: { //HALVEDLOL
				...
				effect() { return Decimal.pow(5, tmp.q.enEff.div(2).max(1).log10().root(1.8)).pow(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("hn"):false)?25:1) },
				effectDisplay() { return format(tmp.hn.upgrades[43].effect)+"x" },
				formula() { return "5^(log(quirkEnergyEff/2)^"+((hasUpgrade("t", 35) && player.i.buyables[12].gte(4))?"0.5605":"0.556")+")" },
			},
			44: { //HALVEDLOL
			...
			45: { //HALVEDLOL
				...
				unlocked() { return hasUpgrade("hn", 53) && hasUpgrade("hn", 54) && player.n.unlocked },
				effect() { return player.n.blueDust.div(2).times(player.n.orangeDust.div(2)).plus(1).log10().plus(1).pow(2) },
				effectDisplay() { return format(tmp.hn.upgrades[45].effect)+"x" },
				formula: "(log(B/2*O/2+1)+1)^2",
			},
			51: { //HALVEDLOL
				...
				effect() { return player.hn.total.div(2).plus(1).pow(3) },
			...
			54: { //HALVEDLOL
				...
				style: {"font-size": "9px"},
				effect() { return player.hs.total.div(2).pow(1.5).plus(1).log10().plus(1).log10().plus(1).log10().times(2).plus(1) },
				effectDisplay() { return format(tmp.hn.upgrades[54].effect.sub(1).times(100))+"% stronger" },
				formula: "log(log(log((x/2)^1.5+1)+1)+1)*200",
			},
			55: { //HALVEDLOL
				...
				unlocked() { return hasUpgrade("hn", 53) && hasUpgrade("hn", 54) && player.n.unlocked },
				effect() { return player.n.orangeDust.div(2).times(player.n.purpleDust.div(2)).plus(1).log10().div(2) },
				effectDisplay() { return "+"+format(tmp.hn.upgrades[55].effect.times(100))+"%" },
				formula: "log(O/2*P/2+1)*50",
			},
		},
})

addLayer("n", { //HALVEDLOL
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?0.05:0.04) }, // Prestige currency exponent //HALVEDLOL
        gainMult() { // Calculate the multiplier for main currency from bonuses //HALVEDLOL
            mult = new Decimal(1);
			...
			if (hasUpgrade("q", 45) && player.i.buyables[12].gte(6)) mult = mult.times(100); //HALVEDLOL
			if (player.ge.unlocked) mult = mult.times(tmp.ge.rotEff);
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("i"):false) mult = mult.times(Decimal.pow(5, player.i.nb)); //HALVEDLOL
			if (hasUpgrade("ai", 24)) mult = mult.times(upgradeEffect("ai", 24));
            return mult
        },
		passiveGeneration() { return (hasMilestone("ma", 3)&&player.ma.current!="n")?0.5:0 }, //HALVEDLOL
        gainExp() { // Calculate the exponent on main currency from bonuses
            return new Decimal(1)
        },
        row: 5, // Row the layer is in on the tree (0 is the first row)
		...
		tabFormat() {  //HALVEDLOL
			...
			["column", 
				...
				["row", [["display-text", ("<span style='color: #bd6afc; font-size: 24px'>"+format(player.n.purpleDust)+"</span> Purple Dust"+(tmp.nerdMode?" (Gain Formula: ((x/2)^0.333)*"+format(tmp.n.dustGainMult.div(20))+")":((tmp.n.effect.purple||new Decimal(1)).lt("1e1000")?(" (+"+format(tmp.n.effect.purple||new Decimal(1))+"/sec)"):""))+"<br><br>Multiply Damned Soul and Phantom Power gain by <span style='color: #bd6afc; font-size: 24px'>"+format(tmp.n.dustEffs.purple)+"</span>"+(tmp.nerdMode?" (Effect Formula: 5^sqrt(log(x/2+1)))":""))]], {"background-color": "rgba(189, 106, 252, 0.25)", width: "50vw", padding: "10px", margin: "0 auto"}],
				
				(second?["column", [["clickable", 11], ["display-text", ("Multiply Magic gain by <span style='color: #ee82ee; font-size: 24px'>"+format(tmp.n.dustEffs2.purpleBlue)+"</span>"+(tmp.nerdMode?" (Effect Formula: (purple/2*blue/2+1)^5)":" (based on Purple & Blue Dust)"))]], {"background-color": "rgba(238, 130, 238, 0.25)", width: "50vw", padding: "10px", margin: "0 auto"}]:[]),
				
				["row", [["display-text", ("<span style='color: #7569ff; font-size: 24px'>"+format(player.n.blueDust)+"</span> Blue Dust"+(tmp.nerdMode?" (Gain Formula: ((x/2)^0.5)*"+format(tmp.n.dustGainMult.div(1e3))+")":((tmp.n.effect.blue||new Decimal(1)).lt("1e1000")?(" (+"+format(tmp.n.effect.blue||new Decimal(1))+"/sec)"):""))+"<br><br>Multiply Super-Booster base by <span style='color: #7569ff; font-size: 24px'>"+format(tmp.n.dustEffs.blue)+"</span>"+(tmp.nerdMode?" (Effect Formula: (x/2+1)^25)":""))]], {"background-color": "rgba(117, 105, 255, 0.25)", width: "50vw", padding: "10px", margin: "0 auto"}],
				
				(second?["column", [["clickable", 12], ["display-text", ("Multiply the <b>Timeless</b> and <b>Option D</b> rewards by <span style='color: #ba9397; font-size: 24px'>"+format(tmp.n.dustEffs2.blueOrange)+"</span><br>(unaffected by softcaps)"+(tmp.nerdMode?" (Effect Formula: (blue/2*orange/2+1)^3)":" (based on Blue & Orange Dust)"))]], {"background-color": "rgba(186, 147, 151, 0.25)", width: "50vw", padding: "10px", margin: "0 auto"}]:[]),
				
				["row", [["display-text", ("<span style='color: #ffbd2e; font-size: 24px'>"+format(player.n.orangeDust)+"</span> Orange Dust"+(tmp.nerdMode?" (Gain Formula: ((x/2)^0.2)*"+format(tmp.n.dustGainMult.div(5))+")":((tmp.n.effect.orange||new Decimal(1)).lt("1e1000")?(" (+"+format(tmp.n.effect.orange||new Decimal(1))+"/sec)"):""))+"<br><br>Multiply amounts of all Solarity buyables by <span style='color: #ffbd2e; font-size: 24px'>"+format(tmp.n.dustEffs.orange)+"</span>"+(tmp.nerdMode?" (Effect Formula: (x/2+1)^38)":""))]], {"background-color": "rgba(255, 189, 46, 0.25)", width: "50vw", padding: "10px", margin: "0 auto"}],
				
				(second?["column", [["clickable", 13], ["display-text", ("Multiply the Time Capsule limit base by <span style='color: #94de95; font-size: 24px'>"+format(tmp.n.dustEffs2.orangePurple)+"</span><br>"+(tmp.nerdMode?" (Effect Formula: (orange/2*purple/2+1)^0.6)":" (based on Orange & Purple Dust)"))]], {"background-color": "rgba(148, 222, 149, 0.25)", width: "50vw", padding: "10px", margin: "0 auto"}]:[]),
			]],
			"blank", "blank", ["buyable", 11], "blank", "blank",
		]},
		dustGainMult() { //HALVEDLOL
			let mult = new Decimal(1);
			if (player.n.buyables[11].gte(1)) mult = mult.times(buyableEffect("o", 22));
			...
			if (player.ge.unlocked) mult = mult.times(tmp.ge.rotEff);
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) mult = mult.times(1e15); //HALVEDLOL
			return mult;
		},
		effect() { //HALVEDLOL
			let amt = player.n.points;
			return {
				purple: amt.div(2).cbrt().div(20).times(tmp.n.dustGainMult),
				blue: amt.div(2).sqrt().div(1e3).times(tmp.n.dustGainMult),
				orange: amt.div(2).root(5).div(5).times(tmp.n.dustGainMult),
			};
		},
		dustProduct() { return player.n.purpleDust.times(player.n.blueDust).times(player.n.orangeDust) },
		dustEffs() { //HALVEDLOL
			let mod = player.n.unlocked?1:0
			let exp = ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.3:1 //HALVEDLOL
			return { //HALVEDLOL
				purple: Decimal.pow(5, player.n.purpleDust.div(2).times(mod).plus(1).log10().sqrt()).pow(exp), //HALVEDLOL
				blue: player.n.blueDust.div(2).times(mod).plus(1).pow(25).pow(exp), //HALVEDLOL
				orange: player.n.orangeDust.div(2).times(mod).plus(1).pow(38).pow(exp), //HALVEDLOL
			}
		},
		dustEffs2() { //HALVEDLOL
			let mod = hasUpgrade("hn", 53)?1:0
			let exp = ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?1.2:1 //HALVEDLOL
			return {
				purpleBlue: player.n.purpleDust.times(player.n.blueDust).div(2).plus(1).pow(5).pow(exp),
				blueOrange: player.n.blueDust.times(player.n.orangeDust).div(2).plus(1).pow(3).pow(exp),
				orangePurple: player.n.orangeDust.times(player.n.purpleDust).div(2).plus(1).pow(0.6).pow(exp),
			}
		},
		realDustEffs2() { //HALVEDLOL
		...
		secondariesAvailable() { return hasUpgrade("hn", 53)?((hasMilestone("hn", 8)&&player.ma.current!="n")?3:(hasUpgrade("hn", 15)?2:1)):0 }, //HALVEDLOL
		secondariesActive() {  //HALVEDLOL
			let n = 0;
...
addLayer("hs", { //HALVEDLOL
        ...
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { 
			let exp = new Decimal(30); //HALVEDLOL
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses //HALVEDLOL
			...
			if (hasUpgrade("s", 33) && player.i.buyables[12].gte(5)) mult = mult.times(upgradeEffect("s", 33));
			if (player.ma.unlocked) mult = mult.times(tmp.ma.effect);
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes("i"):false) mult = mult.times(Decimal.pow(5, player.i.hb)); //HALVEDLOL
            return mult
        },
        gainExp() { // Calculate the exponent on main currency from bonuses
            return new Decimal(1)
		...
		passiveGeneration() { return (hasMilestone("ma", 2)&&player.ma.current!="hs")?0.5:0 }, //HALVEDLOL
        doReset(resettingLayer){ 
			let keep = [];
		...
		buildLimScaling() { return ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?0.9:1 }, //HALVEDLOL
		...
		buildingPower() { //HALVEDLOL
			if (!unl(this.layer)) return new Decimal(0);
			let pow = new Decimal(0.5) //HALVEDLOL
			if (hasUpgrade("hn", 54)) pow = pow.times(upgradeEffect("hn", 54));
			if (player.n.buyables[11].gte(5)) pow = pow.plus(buyableEffect("o", 33));
			if (player.i.buyables[11].gte(5)) pow = pow.plus(buyableEffect("s", 20));
			if (player.ma.unlocked) pow = pow.plus(tmp.ma.effect.max(1).log10().div(80)); //HALVEDLOL
			if (hasAchievement("a", 113)) pow = pow.plus(.05); //HALVEDLOL
			if ((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false) pow = pow.plus(player.hs.buyables[11].div(2000)) //HALVEDLOL
		...
		buyables: {
			...
			21: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
					...
					Primary Space Building Effect: ^"+format(tmp[this.layer].buyables[this.id].effect)+(tmp.nerdMode?" (Formula: level/2*2.5e3+1)":""))
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
				...
				effect() {
					return softcap("hsBuilds", player[this.layer].buyables[this.id]).times(tmp.hs.buildingPower).div(2).times(2.5e3).plus(1);
				},
                buy() { 
			...
			23: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.hs.buildLim)+"\n\n\
					Tertiary Space Building Effect: ^"+format(tmp[this.layer].buyables[this.id].effect)+(tmp.nerdMode?" (Formula: ((level/2)^0.8)*400+1)":""))
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
				...
				effect() {
					return softcap("hsBuilds", player[this.layer].buyables[this.id]).times(tmp.hs.buildingPower).div(2).pow(0.8).times(400).plus(1);
				},
                buy() { 
					player.hs.spentHS = player.hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.hs.buildLim);
			...
			24: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title //HALVEDLOL
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.hs.buildLim)+"\n\n\
					Quaternary Space Building Effect: x"+format(tmp[this.layer].buyables[this.id].effect)+(tmp.nerdMode?" (Formula: ((level/2)^0.8)*1.25e3+1)":" (unaffected by softcap)"))
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
				...
				effect() { //HALVEDLOL
					return softcap("hsBuilds", player[this.layer].buyables[this.id]).times(tmp.hs.buildingPower).div(2).pow(0.8).times(1.25e3).plus(1);
				},
                buy() { 
					player.hs.spentHS = player.hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.hs.buildLim);
			...
			25: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.hs.buildLim)+"\n\n\
					Quinary Space Building Effect: x"+format(tmp[this.layer].buyables[this.id].effect)+(tmp.nerdMode?" (Formula: ((level/2)^0.75)*0.25+1)":""))
					return display;
                },
                unlocked() { return player[this.layer].unlocked }, 
                canAfford() {
				...
				effect() {
					return softcap("hsBuilds", player[this.layer].buyables[this.id]).times(tmp.hs.buildingPower).div(2).pow(0.75).times(0.25).plus(1);
				},
                buy() { 
					player.hs.spentHS = player.hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.hs.buildLim);
			...
			26: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.hs.buildLim)+"\n\n\
					Senary Space Building Effect: ^"+format(tmp[this.layer].buyables[this.id].effect)+(tmp.nerdMode?" (Formula: ((level/2)^1.05)/1.2+1)":""))
					return display;
                },
                unlocked() { return player[this.layer].unlocked && player.i.buyables[11].gte(1) }, 
                canAfford() {
				...
				effect() {
					return softcap("hsBuilds", player[this.layer].buyables[this.id]).times(tmp.hs.buildingPower).div(2).pow(1.05).div(1.2).plus(1);
				},
                buy() { 
					player.hs.spentHS = player.hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.hs.buildLim);
			...
			27: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.hs.buildLim)+"\n\n\
					Septenary Space Building Effect: ^"+format(tmp[this.layer].buyables[this.id].effect)+(tmp.nerdMode?" (Formula: level/10+1)":""))
					return display;
                },
                unlocked() { return player[this.layer].unlocked && player.i.buyables[11].gte(2) }, 
                canAfford() {
				...
				effect() {
					return softcap("hsBuilds", player[this.layer].buyables[this.id]).times(tmp.hs.buildingPower).div(10).plus(1);
				},
                buy() { 
					player.hs.spentHS = player.hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.hs.buildLim);
			...
			28: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.hs.buildLim)+"\n\n\
					Octonary Space Building Effect: x"+format(tmp[this.layer].buyables[this.id].effect)+(tmp.nerdMode?" (Formula: level/2.30+1)":""))
					return display;
                },
                unlocked() { return player[this.layer].unlocked && player.i.buyables[11].gte(3) }, 
                canAfford() {
				...
				effect() {
					return softcap("hsBuilds", player[this.layer].buyables[this.id]).times(tmp.hs.buildingPower).div(2.30).plus(1);
				},
                buy() { 
					player.hs.spentHS = player.hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.hs.buildLim);
			...
			29: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.hs.buildLim)+"\n\n\
					Nonary Space Building Effect: ^"+format(tmp[this.layer].buyables[this.id].effect)+(tmp.nerdMode?" (Formula: level/10+1)":""))
					return display;
                },
			...
			30: { //HALVEDLOL
				...
				display() { // Everything else displayed in the buyable button after the title
                    ...
                    let display = ("Cost: 1 Hyperspace\n\
					Amount: " + formatWhole(player[this.layer].buyables[this.id])+" / "+formatWhole(player.hs.buildLim)+"\n\n\
					Decary Space Building Effect: x"+format(tmp[this.layer].buyables[this.id].effect)+(tmp.nerdMode?" (Formula: sqrt(level/2)/1.5+1)":""))
					return display;
                },
                unlocked() { return player[this.layer].unlocked && player.i.buyables[11].gte(5) }, 
                canAfford() {
				...
				effect() {
					return softcap("hsBuilds", player[this.layer].buyables[this.id]).times(tmp.hs.buildingPower).div(2).sqrt().div(1.5).plus(1);
				},
                buy() { 
					player.hs.spentHS = player.hs.spentHS.plus(1);
					player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1).min(player.hs.buildLim);
...
addLayer("i", { //HALVEDLOL
		...
		base() { return new Decimal(((Array.isArray(tmp.ma.mastered))?tmp.ma.mastered.includes(this.layer):false)?"1e175":"1e250") }, //HALVEDLOL
        gainMult() { // Calculate the multiplier for main currency from bonuses
            mult = new Decimal(1)
            return mult
        },
...
addLayer("ma", { //HALVEDLOL
		...
		rewardDesc() { //HALVEDLOL
			...
			if (player.ma.mastered.includes("ps")) desc += "<h2>Phantom Souls</h2><br><br><ul><li>The Phantom Soul cost base is yroot 1.5'd</li><li>The base Damned Soul gain formula is improved (PS^1.5 -> 1.1^PS)</li><li>The Damned Soul effect is raised ^50</li><li>Wraiths cost scale 25% slower</li><li>Ghost Spirit cost scales 10% slower</li><li>Phantom Boosters are 5% stronger</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("hn")) desc += "<h2>Honour</h2><br><br><ul><li>The Honour gain exponent for its Balance Energy requirement is improved (0.02 -> 0.035)</li><li>Remove the softcap to the second Honour Upgrade</li><li><b>Self-Self-Synergy</b>'s effect is multiplied by 2</li><li><b>Point Efficiency</b> is maxed at 45.5% instead of 45%</li><li><b>Superpowered Upgrades</b>'s effect is x1.5'd</li><li><b>Reversal Sensational</b> is 5% stronger</li><li><b>Column Leader Leader</b> is 5% stronger</li><li><b>Again and Again</b>'s effect is x1.5'd</li><li><b>Quir-cursion</b>'s effect is raised ^25</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("n")) desc += "<h2>Nebula</h2><br><br><ul><li>The Nebula gain exponent is improved (0.03 -> 0.04)</li><li>All primary dust effects are raised ^1.3</li><li>All secondary dust effects are raised ^1.2</li><li>Gain 1e15x more Dust</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("hs")) desc += "<h2>Hyperspace</h2><br><br><ul><li>The Hyper Building Limit requirement scales 10% slower</li><li>Add 0.1% to Hyper Building Power for every Hyperspace bought</li><li>The Hyper Building softcap starts 0.05 Levels later</li></ul><br><br>"; //HALVEDLOL
			if (player.ma.mastered.includes("i")) desc += "<h2>Imperium</h2><br><br><ul><li>The Imperium Building cost base is reduced (1e250 -> 1e175)</li><li>Each Nebulaic Brick multiplies Nebula Energy gain by 5</li><li>Each Hyperspatial Brick multiplies Hyperspace Energy gain by 5</li><li>There are 2 new Imperium Buildings</li></ul><br><br>"; //HALVEDLOL
			return desc;
		},
		milestones: { //HALVEDLOL
			0: {
			...
			1: {
				...
				effectDescription: "You can buy max Imperium Bricks (which now reset nothing), gain 50% of Honour gain every second, and keep all Honour milestones & upgrades on all resets.",
			},
			2: {
				requirementDescription: "3 Mastery",
				done() { return player.ma.best.gte(3) },
				effectDescription: "Keep Imperium Building I & Hyper Buildings on reset, and gain 50% of Hyperspace Energy gain every second.",
			},
			3: {
				requirementDescription: "4 Mastery",
				done() { return player.ma.best.gte(4) },
				effectDescription: "Gain 50% of Nebula Energy every second.",
			},
			4: {
...
addLayer("ge", { //HALVEDLOL
        ...
        gainExp() { // Calculate the exponent on main currency from bonuses //HALVEDLOL
            let exp = new Decimal(1)
			if (hasUpgrade("ai", 34)) exp = exp.times(1.1);
			return exp;
        },
        row: 6, // Row the layer is in on the tree (0 is the first row)
        hotkeys: [
		...
		passiveGeneration() { return hasMilestone("ge", 2)?0.005:0 }, //HALVEDLOL
        doReset(resettingLayer){ 
			let keep = [];
			if (layers[resettingLayer].row > this.row) layerDataReset(this.layer, keep)
			if (layers[resettingLayer].row >= this.row) {
		...
		tabFormat: ["main-display",
			...
			"milestones",
			"blank", "blank", 
			["display-text", function() { return "<h3>Gear Speed: "+format(tmp.ge.gearSpeed)+"x</h3>"+(tmp.nerdMode?" (cbrt(gears/2))":"") }],
			"blank",
			["display-text", function() { return "<b>Gear Radius: "+format(tmp.ge.radius)+"m</b>"+(tmp.nerdMode?" (teeth*toothSize/3.14)":"") }], "blank",
			["row", [["display-text", function() { return "<h3>Rotations: "+formatWhole(player.ge.rotations, true)+" ("+tmp.ge.rotDesc+")</h3><br>Rotation Effect: Multiply Nebula Energy & Dust gain by "+format(tmp.ge.rotEff)+(tmp.nerdMode?" ((x/2+1)^3)":"") }]]],
			"blank", "blank",
			["clickable", 21],
			"blank", "blank",
		...
		gearSpeed() { //HALVEDLOL
			let speed = player.ge.points.div(2).cbrt().times(player.mc.unlocked?tmp.mc.mechEff:1);
			if (player.mc.upgrades.includes(11)) speed = speed.times(buyableEffect("mc", 12));
			return speed;
		},
		rps() {
			return tmp.ge.speed.div(2).div(tmp.ge.teeth.times(tmp.ge.toothSize).mul(4)).times(tmp.ge.gearSpeed.div(2))
		},
		rotDesc() {
			let rps = tmp.ge.rps;
			let desc = "";
			...
			if (tmp.nerdMode) desc += " </h3>((velocity/2*gearSpeed/2)/(radius/2*6.28))<h3>"
			return desc;
		},
		speed() { //HALVEDLOL
			return player.ge.energy.div(2).sqrt();
		},
		teeth() {
		...
		toothSize() { //HALVEDLOL
			return player.ge.shrinkPower.div(2).plus(1).pow(-0.5).div(tmp.ge.clickables[13].effect).times(player.mc.unlocked?tmp.mc.buyables[11].effect.pow(hasAchievement("a", 125)?(-0.5):0.5):0.5);
		},
		radius() { return tmp.ge.teeth.times(tmp.ge.toothSize).div(Math.PI) }, //HALVEDLOL
		boostReducedPurch() { return tmp.ge.buyables[11].effect.times(2) }, //HALVEDLOL
		boostReq() { 
		...
		buyables: { //HALVEDLOL
			...
			11: {
				...
				free() {
					let free = new Decimal(0);
					if (hasAchievement("a", 132)) free = free.plus(1); //HALVEDLOL
					return free;
				},
				power() {
					let pow = new Decimal(1);
					if (hasAchievement("a", 124)) pow = pow.times(1.1); //HALVEDLOL
					if (hasUpgrade("ai", 14)) pow = pow.times(1.056); //HALVEDLOL
					return pow;
				},
				cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
				...
				effectPer() { return Decimal.div(tmp.ge.buyables[this.id].power, 1.5) }, //HALVEDLOL
				effect() { return Decimal.mul(tmp[this.layer].buyables[this.id].effectPer, player[this.layer].buyables[this.id].div(2).plus(tmp.ge.buyables[this.id].free.div(2)).times(hasUpgrade("ai", 13)?1.25:1)) }, //HALVEDLOL
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id];
					let cost = data.cost;
					let amt = player[this.layer].buyables[this.id];
		...
		clickables: {
			...
			11: { //HALVEDLOL
				...
				effectPer() { return Decimal.add(1, tmp.ge.buyables[11].effect.div(2)) }, //HALVEDLOL
				effect() { return Decimal.pow(tmp.ge.clickables[this.id].effectPer.div(2), player.ge.clickables[this.id].div(2)) }, //HALVEDLOL
			...
			12: { //HALVEDLOL
				...
				effectPer() { return Decimal.add(3, tmp.ge.buyables[11].effect).times(hasAchievement("a", 123)?2:1) }, //HALVEDLOL
				effect() { return Decimal.pow(tmp.ge.clickables[this.id].effectPer.div(2), player.ge.clickables[this.id].div(2)) }, //HALVEDLOL
			...
			13: { //HALVEDLOL
				...
				effectPer() { return Decimal.add(1, tmp.ge.buyables[11].effect.div(2)) },
				effect() { return Decimal.pow(tmp.ge.clickables[this.id].effectPer.div(2), player.ge.clickables[this.id].div(2)) },
				unlocked() { return player.ge.unlocked },
				canClick() { return player.ge.unlocked && tmp.n.dustProduct.gte(tmp.ge.clickables[this.id].req) },
				onClick() { 
					if (player.ge.maxToggle && hasMilestone("ge", 0)) {
		...
		milestones: { //HALVEDLOL
			...
			2: { //HALVEDLOL
				...
				effectDescription: "Total Energy multiplies Gear gain, and gain 0.5% of Gear gain every second.",
			},
			3: {
...
addLayer("mc", { //HALVEDLOL
		...
		passiveGeneration() { return hasMilestone("mc", 0)?0.005:0 }, //HALVEDLOL
        doReset(resettingLayer){ 
			let keep = [];
			if (layers[resettingLayer].row > this.row) layerDataReset(this.layer, keep)
        },
		...
		decayPower() { return player.mc.mechEn.div(2).plus(1).log10().div(hasUpgrade("ai", 31)?1.5:1).plus(1).pow(-2) }, //HALVEDLOL
		mechEff() { return Decimal.pow(5, player.mc.mechEn.div(2).plus(1).log10().root(4).div(2)) },
		tabFormat: {
			"The Shell": {
			...
			"The Motherboard": {
				...
				"blank", ["display-text", function() { return "Your Mech-Energy is multiplying Gear Speed by "+format(tmp.mc.mechEff)+(tmp.nerdMode?" (Formula: 5^((log(x/2+1)^0.25)/2))":"") }],
		...
		milestones: {
			0: { //HALVEDLOL
				...
				effectDescription: "Thoughts multiply Machine Part gain, and gain 0.5% of Machine Part gain every second.",
			},
			1: { //HALVEDLOL
				...
				effectDescription: "CPU's effect is raised ^13, and unlock Auto-Motherboard.",
				toggles: [["mc", "auto"]],
			},
		},
		clickables: { //HALVEDLOL
			rows: 2,
			cols: 2,
			...
			11: { //HALVEDLOL
				title: "CPU",
				display() { 
					return "Active Mech-Energy: "+format(player.mc.clickables[this.id])+"<br><br>Currently: Kinetic Energy multiplies Mech-Energy gain by "+format(tmp.mc.clickables[this.id].effect)+(tmp.nerdMode?" (Formula: (kineticEnergy/2+1)^(1-1/sqrt(log(activeMechEnergy/2+1)+1)))":"");
				},
				effect() { 
					let eff = Decimal.pow(player.ge.energy.div(2).plus(1), Decimal.sub(1, Decimal.div(1, Decimal.add(player.mc.clickables[this.id].div(2), 1).log10().plus(1).sqrt())));
					if (hasMilestone("mc", 1)) eff = eff.pow(13); //HALVEDLOL
			...
			12: { //HALVEDLOL
				title: "The Port",
				display() { 
					return "Active Mech-Energy: "+format(player.mc.clickables[this.id])+"<br><br>Currently: Phantom Souls multiply Gear gain by "+format(tmp.mc.clickables[this.id].effect)+(tmp.nerdMode?" (Formula: (phantomSouls/2+1)^(1-1/sqrt(log(activeMechEnergy/2+1)+1)))":"");
				},
				effect() { return Decimal.pow(player.ps.points.div(2).plus(1), Decimal.sub(1, Decimal.div(1, Decimal.add(player.mc.clickables[this.id].div(2), 1).log10().plus(1).sqrt()))) },
				unlocked() { return player.mc.unlocked },
				canClick() { return player.mc.unlocked },
				onClick() {
					if (player.mc.clickables[this.id].eq(0)) {
			...
			21: { //HALVEDLOL
				title: "Northbridge",
				display() { 
					return "Active Mech-Energy: "+format(player.mc.clickables[this.id])+"<br><br>Currently: Solarity multiplies the Super Generator base by "+format(tmp.mc.clickables[this.id].effect)+(tmp.nerdMode?" (Formula: (solarity/2+1)^("+formatWhole(tmp.mc.clickables[this.id].effExp)+"-"+formatWhole(tmp.mc.clickables[this.id].effExp)+"/((log(activeMechEnergy/2+1)+1)^0.125)))":"");
				},
				effExp() { return hasAchievement("a", 133)?2:1 }, //HALVEDLOL
				effect() { return Decimal.pow(player.o.points.div(2).plus(1), Decimal.sub(tmp.mc.clickables[this.id].effExp, Decimal.div(tmp.mc.clickables[this.id].effExp, Decimal.add(player.mc.clickables[this.id].div(2), 1).log10().plus(1).root(8)))) },
				unlocked() { return player.mc.unlocked },
				canClick() { return player.mc.unlocked },
				onClick() {
					if (player.mc.clickables[this.id].eq(0)) {
			...
			22: { //HALVEDLOL
				title: "Southbridge",
				display() { 
					return "Active Mech-Energy: "+format(player.mc.clickables[this.id])+"<br><br>Currently: Hyperspace Energy multiplies Balance Energy gain by "+format(tmp.mc.clickables[this.id].effect)+(tmp.nerdMode?" (Formula: (hyperspaceEnergy/2+1)^(1-1/cbrt(log(activeMechEnergy/2+1)+1)))":"");
				},
				effect() { return Decimal.pow(player.hs.points.div(2).plus(1), Decimal.sub(1, Decimal.div(1, Decimal.add(player.mc.clickables[this.id].div(2), 1).log10().plus(1).cbrt()))) },
				unlocked() { return player.mc.unlocked },
				canClick() { return player.mc.unlocked },
				onClick() {
					if (player.mc.clickables[this.id].eq(0)) {
		...
		buyables: {
			...
			11: { //HALVEDLOL
				...
				buffExp() {  //HALVEDLOL
					...
					if (hasUpgrade("ai", 33)) exp *= 50;
					return exp;
				},
				effect() { return player[this.layer].buyables[this.id].div(2).plus(1).sqrt() },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id];
					let cost = data.cost;
					let amt = player[this.layer].buyables[this.id];
                    let display = "Req: "+formatWhole(cost)+" Machine Parts"+(tmp.nerdMode?" (Cost Formula: floor((x/10+0.5)^1.1)":"")+".<br><br><h3>Current Shell Size: "+formatWhole(amt)+"m</h3>, which multiplies Mech-Energy gain by "+format(data.effect.pow(data.buffExp))+(tmp.nerdMode?" (Formula: (x/2+1)^1.75)":"")+" "+(hasAchievement("a", 125)?"and also divides":"but also multiplies")+" Tooth Size of Gears by "+format(data.effect)+(tmp.nerdMode?" (Formula: sqrt(x/2+1))":"");
					return display;
                },
                unlocked() { return unl(this.layer) }, 
                canAfford() {
...
addLayer("en", { //HALVEDLOL
		...
		gainMult() { //HALVEDLOL
			let mult = new Decimal(1);
			if (hasMilestone("en", 0)) mult = mult.times(1.5); //HALVEDLOL
			if (hasMilestone("en", 2)) mult = mult.times(player.o.points.div(2).plus(1).log10().plus(1).log10().plus(1)); //HALVEDLOL
		...
		passiveGeneration() { return hasMilestone("en", 0)?0.05:0 }, //HALVEDLOL
		canReset() {
		...
		update(diff) {
			...
			if (player.en.points.gt(0)) { //HALVEDLOL
				...
				if (hasMilestone("en", 1)) player.en.stored = player.en.stored.plus(subbed.div(10)); //HALVEDLOL
			}
			let sw_mw_exp = hasUpgrade("ai", 34)?0.9:1 //HALVEDLOL
			if (hasMilestone("r", 1)) {
				subbed = subbed.times(player.r.total.div(3).max(1));
				if (hasMilestone("r", 4) && tmp.r) subbed = subbed.times(tmp.r.producerEff.div(2).max(1)); //HALVEDLOL
				player.en.tw = player.en.tw.pow(1.25).plus(subbed.div(player.en.target==1?1:3)).root(1.25); //HALVEDLOL
				player.en.ow = player.en.ow.pow(1.25).plus(subbed.div(player.en.target==2?1:3)).root(1.25); //HALVEDLOL
				player.en.sw = player.en.sw.pow(sw_mw_exp*(hasMilestone("en", 4)?3.25:4)).plus(subbed.div(player.en.target==3?1:3)).root(sw_mw_exp*(hasMilestone("en", 4)?3.25:4)); //HALVEDLOL
				if (hasMilestone("en", 3)) player.en.mw = player.en.mw.pow(sw_mw_exp*(hasMilestone("en", 4)?6.25:7)).plus(subbed.div(player.en.target==4?1:3)).root(sw_mw_exp*(hasMilestone("en", 4)?6.25:7)); //HALVEDLOL
				
			} else switch(player.en.target) {
				case 1: 
					player.en.tw = player.en.tw.pow(1.25).plus(subbed).root(1.25); //HALVEDLOL
					break;
				case 2: 
					player.en.ow = player.en.ow.pow(1.25).plus(subbed).root(1.25); //HALVEDLOL
					break;
				case 3: 
					player.en.sw = player.en.sw.pow(sw_mw_exp*(hasMilestone("en", 4)?3.25:4)).plus(subbed).root(sw_mw_exp*(hasMilestone("en", 4)?3.25:4)); //HALVEDLOL
					break;
				case 4: 
					if (hasMilestone("en", 3)) player.en.mw = player.en.mw.pow(sw_mw_exp*(hasMilestone("en", 4)?6.25:7)).plus(subbed).root(sw_mw_exp*(hasMilestone("en", 4)?6.25:7)); //HALVEDLOL
					break;
			}
		},
		storageLimit() { return player.en.total.div(4) }, //HALVEDLOL
		twEff() { return player.en.tw.div(2).plus(1).log10().plus(1).log10().times(5).plus(1).pow(2) }, //HALVEDLOL
		owEff() { return player.en.ow.div(2).plus(1).log10().plus(1).log10().times(20).pow(1.4) }, //HALVEDLOL
		swEff() { return player.en.sw.div(2).plus(1).log10().plus(1).log10().plus(1).log10().plus(1) }, //HALVEDLOL
		mwEff() { return hasMilestone("en", 3)?player.en.mw.div(2).plus(1).log10().plus(1).log10().div(10).plus(1).pow(1.5):new Decimal(1) }, //HALVEDLOL
		tabFormat: ["main-display",
			"prestige-button",
			"resource-display", "blank",
			"milestones",
		...
		milestones: { //HALVEDLOL
			0: { //HALVEDLOL
				...
				effectDescription: "Gain 5% of Energy gain every second, you can always Energy reset when under 100% of Energy gain, and Energy gain is x1.5'd.",
			},
			1: { //HALVEDLOL
				requirementDescription: "22,500 Energy in one reset",
				done() { return player.en.bestOnReset.gte(22500) || hasAchievement("a", 151) },
				effectDescription: "10% of Energy that's lost over time becomes stored, Energy milestones are kept on all resets up to Row 7 (except Robots & AI), and when below 1, the Stored Energy effect is yroot 1.5'd.",
			},
			2: { //HALVEDLOL
				requirementDescription: "335,000 Energy in one reset",
				done() { return player.en.bestOnReset.gte(335e3) || hasAchievement("a", 151) },
				effectDescription() { return "Energy gain is multiplied by the double-log of your Solarity ("+format(player.o.points.div(2).plus(1).log10().plus(1).log10().plus(1))+"x)." },
			},
			3: {
			...
			4: { //HALVEDLOL
				...
				effectDescription() { return "The Mind Watt & Super Watt gain roots are decreased by 0.75" },
			},
		},
		clickables: {
			rows: 1,
			...
			11: { //HALVEDLOL
				title: "Store Energy",
				display(){
					return "Stored Energy: <span style='font-size: 20px; font-weight: bold;'>"+formatWhole(player.en.stored)+" / "+formatWhole(tmp.en.storageLimit)+"</span><br><br>"+(tmp.nerdMode?("Effect Formula: log(log(x/2+1)+1)/10"):("Increases Energy gain exponent by <span style='font-size: 20px; font-weight: bold;'>"+format(tmp.en.clickables[11].eff)+"</span>"))
				},
				eff() { 
					let e = player.en.stored.div(2).plus(1).log10().plus(1).log10().div(10);
					if (hasMilestone("en", 1) && e.lt(1)) e = e.root(1.5); //HALVEDLOL
					return e;
				},
				unlocked() { return player.en.unlocked },
...
addLayer("ne", { //HALVEDLOL
		...
		effect() { //HALVEDLOL
			let eff = player[this.layer].points.div(4).plus(1).pow(0.75).sub(1); //HALVEDLOL
			if (hasMilestone("ne", 3)) eff = eff.times(Decimal.pow(1.25, player[this.layer].points.div(2).sqrt()).plus(player[this.layer].points.div(2))); //HALVEDLOL
			if (hasMilestone("ne", 6)) eff = eff.pow(1.5); //HALVEDLOL
			if (hasMilestone("id", 1)) eff = eff.pow(1.5).times(player[this.layer].buyables[11].div(2).max(1)); //HALVEDLOL
			return eff;
		},
		...
		signalLimThresholdInc() { //HALVEDLOL
			let inc = new Decimal(hasMilestone("ne", 4)?3.5:(hasMilestone("ne", 3)?3.75:(hasMilestone("ne", 2)?4:5)));
		...
		thoughtPower() { //HALVEDLOL
			let pow = new Decimal(1);
			if (player.en.unlocked && hasMilestone("en", 3)) pow = pow.times(tmp.en.mwEff);
			if (hasMilestone("id", 1)) pow = pow.times(1.1); //HALVEDLOL
			return pow;
		},
		thoughtEff1() { return player.ne.thoughts.times(tmp.ne.thoughtPower).div(4).plus(1).log10().plus(1).pow(hasMilestone("ne", 1)?1.5:1) }, //HALVEDLOL
		thoughtEff2() { return Decimal.pow("1e400", player.ne.thoughts.times(tmp.ne.thoughtPower).div(4).pow(.75)).pow(hasMilestone("ne", 2)?1.5:1) }, //HALVEDLOL
		thoughtEff3() { return Decimal.pow(1.1, player.ne.thoughts.div(2).times(hasMilestone("ne", 5)?tmp.ne.thoughtPower.div(2):0).sqrt()) }, //HALVEDLOL
		challenges: { //HALVEDLOL*
			rows: 1,
			cols: 1,
			11: {
				name: "The Brain",
				...
				currencyInternalName: "points",
				gainMult() {  //HALVEDLOL
					let mult = tmp.ne.effect.times(player.ne.signals.div(2).plus(1).log10().plus(1)); //HALVEDLOL
					if (hasMilestone("ne", 0)) mult = mult.times(player.ss.points.div(2).plus(1).sqrt()); //HALVEDLOL
					if (hasMilestone("ne", 2)) mult = mult.times(player.ne.points.max(1));
					if (player.en.unlocked && hasMilestone("en", 3)) mult = mult.times(tmp.en.mwEff.pow(20)); //HALVEDLOL
					if (hasAchievement("a", 143)) mult = mult.times(2); //HALVEDLOL
					if (hasMilestone("r", 0)) mult = mult.times(player.r.maxMinibots.max(1));
					if (hasMilestone("r", 4) && tmp.r) mult = mult.times(tmp.r.producerEff.max(1));
					if (hasMilestone("id", 3) && tmp.mc) mult = mult.times(Decimal.pow(1.5, player.mc.buyables[11].div(2).max(1).log10()));
					if (player.ai.unlocked && tmp.ai) mult = mult.times(tmp.ai.conscEff1);
					if (player.c.unlocked && tmp.c) mult = mult.times(tmp.c.eff3);
					if (hasUpgrade("ai", 42)) mult = mult.times(upgradeEffect("ai", 42)); //HALVEDLOL
					return mult;
				},
				amt() { //i hope this won't break the game
					let a = Decimal.pow(5, player.points.div(2).plus(1).log10().plus(1).log10().div(player.ne.activeChallenge==11?22:28).pow(2)).pow(tmp.ne.buyables[11].effect.div(2)).times(tmp.ne.challenges[11].gainMult.div(2)).floor();
				...
				rewardDescription() { return "<br>Signals: <h3 style='color: #ded9ff'>"+formatWhole(player.ne.signals)+"/"+formatWhole(tmp.ne.signalLim)+"</h3> "+(tmp.nerdMode?("(Gain Formula: 5^((log(log(points/2+1)+1)/22)^2)*"+format(tmp.ne.challenges[11].gainMult)+")"):("(+"+formatWhole((player.ne.activeChallenge==11||hasAchievement("a", 151))?tmp.ne.challenges[11].amt:0)+"/s"+(tmp.ne.challenges[11].amt.lt(1e3)?(", next gain at "+format(tmp.ne.challenges[11].next)+" Points)"):")")))+"<br><br><br>Thoughts: <h3 style='color: #ffbafa'>"+formatWhole(player.ne.thoughts)+"</h3> (Next at "+formatWhole(tmp.ne.signalLim)+" Signals)<br><br>Effects"+(tmp.ne.thoughtPower.eq(1)?"":(" (Power: "+format(tmp.ne.thoughtPower.times(100))+"%)"))+"<br>Cheapen Subspace Energy by "+(tmp.nerdMode?" (Formula: (log(thoughts/2+1)+1)"+(hasMilestone("ne", 1)?"^1.5":"")+")":(format(tmp.ne.thoughtEff1)+"x"))+"<br>Multiply Subspace & SG bases by "+(tmp.nerdMode?" (Formula: (1e400^((thoughts/2)^0.75))"+(hasMilestone("ne", 2)?"^2":"")+")":format(tmp.ne.thoughtEff2)+"x")+(hasMilestone("ne", 5)?("<br>Multiply Energy gain by "+(tmp.nerdMode?" (Formula: (1.1^sqrt(thoughts/2)))":(format(tmp.ne.thoughtEff3)+"x"))):"") },
		...
		buyables: { //HALVEDLOL
			...
			11: {
				...
				ss() { return hasMilestone("id", 0)?11:10 }, //HALVEDLOL
				cost(x=player[this.layer].buyables[this.id]) {
				...
				power() { //HALVEDLOL
					let p = new Decimal(hasUpgrade("ai", 11)?1.25:1);
					if (player.c.unlocked && tmp.c) p = p.times(tmp.c.eff5);
					return p;
				},
				effect() { return player[this.layer].buyables[this.id].times(tmp.ne.buyables[11].power).div(6).plus(1) },
				display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id];
					let cost = data.cost;
					let amt = player[this.layer].buyables[this.id];
                    let display = "Cost: "+format(cost)+" Signals"+(tmp.nerdMode?(" (Cost Formula: 4^("+(amt.gte(data.ss)?(formatWhole(data.ss)+"^(log"+formatWhole(data.ss)+"(x)^"+format(hasMilestone("id", 0)?1.587:2)+")"):"x")+"^1.2)*2e4)"):"")+".<br><br>Level: "+formatWhole(amt)+"<br><br>Effect: Signal gain from Points is raised ^"+format(data.effect)+(tmp.nerdMode?" (Formula: x/6+1)":""); //HALVEDLOL
					return display;
                },
		...
		milestones: {
			0: { //HALVEDLOL
				...
				effectDescription() { return "Subspace Energy multiplies Signal gain ("+format(player.ss.points.div(2).plus(1).sqrt())+"x), and unlock The Neural Network" },
			},
			1: { //HALVEDLOL
				requirementDescription: "50,000 Signals",
				done() { return player.ne.signals.gte(5e4) || player.ne.milestones.includes(1) },
				effectDescription() { return "The first Thought effect is ^1.5'd, and Neuron milestones are kept on all resets up to Row 7 (except Ideas)" },
			},
			2: { //HALVEDLOL
				requirementDescription: "3,000,000 Signals",
				done() { return player.ne.signals.gte(3e6) || player.ne.milestones.includes(2) },
				effectDescription() { return "The Thought requirement increases slower (5x -> 4x), the second Thought effect is ^1.5'd, and multiply Signal gain by your Neurons" },
			},
			3: { //HALVEDLOL
				requirementDescription: "150,000,000 Signals",
				done() { return player.ne.signals.gte(1.5e8) || player.ne.milestones.includes(3) },
				effectDescription() { return "The Thought requirement increases even slower (4x -> 3.75x), and the Neuron effect uses a better formula (becomes exponential instead of sub-linear)" },
			},
			4: { //HALVEDLOL
				requirementDescription: "2.5e9 Signals",
				done() { return player.ne.signals.gte(2.5e9) || player.ne.milestones.includes(4) },
				effectDescription() { return "The Thought requirement increases even slower (3.75x -> 3.5x), and getting a Thought does not reset Signals" },
			},
			5: { //HALVEDLOL
				unlocked() { return player.en.unlocked && player.ne.unlocked },
			...
			6: { //HALVEDLOL
				...
				effectDescription() { return "The Neuron effect is ^1.5'd & multiplied by your Neural Network level" },
			},
			7: { //HALVEDLOL
...
addLayer("id", { //HALVEDLOL
		...
		effect() { return Decimal.sub((hasAchievement("a", 155)?0.0025:0)+(hasUpgrade("ai", 32)?0.97:0.95), Decimal.div(0.95, player.id.points.div(2).plus(1).log10().times(hasMilestone("id", 4)?1.25:1).times(hasMilestone("id", 5)?1.375:1).plus(1))) }, //HALVEDLOL
		effectDescription() { return "which reduce the Thought threshold's increase by <h2 style='color: #fad682; text-shadow: #fad682 0px 0px 10px;'>"+format(tmp[this.layer].effect)+"</h2>"+(tmp.nerdMode?" (0.95-0.95/(log(x/2+1)+1)).":".") },
		rev() { return player.ne.signals.div(2).plus(1).log10().div(20).pow(.75).times(player.id.points.div(2)).pow(hasMilestone("id", 0)?1.5:1).times(hasUpgrade("ai", 32)?1.25:1).times(hasUpgrade("ai", 14)?1.25:1).floor() }, //HALVEDLOL
		revEff() { return Decimal.pow(5e12, tmp.id.rev.pow(.95)) }, //HALVEDLOL
        gainMult() { // Calculate the multiplier for main currency from bonuses //HALVEDLOL
            mult = new Decimal(1)
			if (hasMilestone("id", 2)) mult = mult.div(player.ne.points.div(2).plus(1).log10().plus(1));
            return mult
        },
        gainExp() { // Calculate the exponent on main currency from bonuses
            return new Decimal(1)
        ...
        doReset(resettingLayer){ 
            ...
            if (layers[resettingLayer].row > this.row) layerDataReset(this.layer, keep)
			if (hasUpgrade("ai", 22) && !(layers[resettingLayer].row<7&&resettingLayer!="ai"&&resettingLayer!="c")) addPoints("id", 2);
        },
		autoPrestige() { return hasMilestone("id", 4) && player.id.auto },
        layerShown(){return player.en.unlocked&&player.ne.unlocked},
        branches: ["ne"],
		tabFormat: ["main-display", //HALVEDLOL
			"prestige-button",
			"resource-display", "blank", 
			"milestones", "blank", "blank",
			["display-text", function() { return "Revelations: <h2>"+formatWhole(tmp.id.rev)+"</h2>"+(tmp.nerdMode?(hasMilestone("id", 0)?" (((ideas/2)^2)*(log(signals/2+1)/10)^1.25)":" (ideas/2*(log(signals/2+1)/20)^0.75)"):" (based on Ideas & Signals)") }],
			["display-text", function() { return "Effect: Multiply Mech-Energy by <h2>"+format(tmp.id.revEff)+"</h2>"+(tmp.nerdMode?" (5e12^((x/2)^0.95))":".") } ], "blank",
		],
		milestones: {
			0: { //HALVEDLOL
				requirementDescription: "2 Ideas & 2 Revelations",
				done() { return (player.id.points.gte(2) && tmp.id.rev.gte(2))||hasAchievement("a", 161) },
				effectDescription: "Neural Network cost scaling starts 1 purchases later & is 25% weaker, Thoughts can be gained in bulk, and Revelations are ^1.25'd.",
			},
			1: { //HALVEDLOL
				...
				effectDescription: "The Neuron effect is ^1.5'd, and Thought effects are all 10% stronger.",
			},
			2: { //HALVEDLOL
				...
				effectDescription() { return "The Idea cost is divided based on your Neurons (/"+format(player.ne.points.div(2).plus(1).log10().plus(1))+")" },
			},
			3: { //HALVEDLOL
				...
				effectDescription() { return "Unlock Auto-Shell Expansion, which is much more efficient than normal purchasing, and each OoM of Shell Expansions *1.5 Signal gain ("+format(Decimal.pow(1.5, player.mc.buyables[11].max(1).log10()))+"x)." },
				toggles: [["mc", "autoSE"]],
			},
			4: { //HALVEDLOL
				...
				effectDescription: "Unlock Auto-Ideas, you can buy max Ideas, & the Idea effect is 25% more effective.",
				toggles: [["id", "auto"]],
			},
			5: { //HALVEDLOL
...
addLayer("r", { //HALVEDLOL
        ...
        startData() { return {
			...
			total: new Decimal(0),
			allotted: { //HALVEDLOL
				breeders: new Decimal((player && tmp.ma && hasUpgrade("ai", 12))?2.5:0),
				farmers: new Decimal((player && tmp.ma && hasUpgrade("ai", 12))?2.5:0),
				builders: new Decimal((player && tmp.ma && hasUpgrade("ai", 12))?2.5:0),
				growers: new Decimal((player && tmp.ma && hasUpgrade("ai", 12))?2.5:0),
				producers: new Decimal((player && tmp.ma && hasUpgrade("ai", 12))?2.5:0),
			},
		...
		gainMult() { //HALVEDLOL
			let mult = new Decimal(1);
			if (hasMilestone("r", 3)) mult = mult.times(1.5); //HALVEDLOL
			if (player.ai.unlocked && tmp.ai) mult = mult.times(tmp.ai.conscEff1); //HALVEDLOL
			if (hasUpgrade("ai", 33)) mult = mult.times(upgradeEffect("ai", 33)); //HALVEDLOL
			return mult;
		},
		getResetGain() {
		...
		update(diff) { //HALVEDLOL
			if (!player[this.layer].unlocked) return;
			player.r.maxMinibots = player.r.maxMinibots.div(2).max(tmp.r.totalMinibots.div(2)); //HALVEDLOL
			player.r.fuel = player.r.fuel.div(2).pow(1.25).plus(player.r.allotted.farmers.div(8).times(diff)).root(1.5); //HALVEDLOL
			player.r.buildings = player.r.buildings.div(2).pow(1.5).plus(player.r.allotted.builders.div(2).times((hasMilestone("id", 5)&&tmp.id)?tmp.id.rev.max(1):1).div(6).times(diff)).sqrt(); //HALVEDLOL
			...
			if (Decimal.gte(player.r.growTime, tmp.r.growTime)) {
				...
				player.r.growTime = new Decimal(0);
				if (tmp.r.minibots.gt(0)) {
					addPoints("r", hasUpgrade("ai", 12)?bulk.times(tmp.r.getResetGain.div(40)):bulk); //HALVEDLOL
					player.r.spentMinibots = player.r.spentMinibots.plus(bulk);
					player.r.grownMinibots = player.r.grownMinibots.plus(bulk);
				}
			}
			if (hasMilestone("r", 5)) {
				player.r.allotted.breeders = player.r.allotted.breeders.plus(player.r.points.div(100).times(diff));
				player.r.allotted.farmers = player.r.allotted.farmers.plus(player.r.points.div(100).times(diff));
				player.r.allotted.builders = player.r.allotted.builders.plus(player.r.points.div(100).times(diff));
				player.r.allotted.growers = player.r.allotted.growers.plus(player.r.points.div(100).times(diff));
				player.r.allotted.producers = player.r.allotted.producers.plus(player.r.points.div(100).times(diff));
			}
		},
		tabFormat: ["main-display", //HALVEDLOL
			"prestige-button",
			...
			["row", [
				...
				["column", [
					...
					["display-text", function() { return "Buildings: "+formatWhole(player.r.buildings.floor())+", which caps your Minibots at "+formatWhole(tmp.r.minibotCap)+(tmp.nerdMode?" (Formula: log2(x/2)+1.5)":"")+" and multiplies Gear gain by "+formatWhole(tmp.r.buildingEff)+(tmp.nerdMode?" (Formula: (x/2-1)^2*50+1)":".") }],
				], {width: "9em"}],
				["tall-display-text", "<div class='vl2'></div>", {height: "223.667px"}],
				["column", [
				...
				["column", [
					...
					["display-text", function() { return "Multiply Energy gain by "+format(tmp.r.producerEff)+(tmp.nerdMode?" (Formula: (((x/2)^1.25)/8+1))":"") }],
				], {width: "9em"}],
		...
		breederExp() { //HALVEDLOL
			let exp = new Decimal(2);
			if (hasMilestone("r", 2)) exp = exp.times(1.5);
			return exp;
		},
		reduceMinibotReqMult() { //HALVEDLOL
			let mult = new Decimal(0);
			if (hasMilestone("r", 3)) mult = mult.plus(.125); //HALVEDLOL
			if (hasUpgrade("ai", 23)) mult = mult.plus(.25); //HALVEDLOL
			return mult;
		},
		nextMinibot() { 
		...
		totalMinibots() {  //HALVEDLOL
			if (player.r.allotted.breeders.lt(1)) return new Decimal(0);
			else return player.en.total.div(2).times(player.r.allotted.breeders.div(2).pow(tmp.r.breederExp)).div(2e5).max(1).log10().root(1.5).plus(player.r.grownMinibots.times(tmp.r.reduceMinibotReqMult)).floor().min(tmp.r.minibotCap.plus(player.r.spentMinibots))
		},
		minibots() { return player.r.maxMinibots.sub(player.r.spentMinibots).max(0) },
		deathTime() { return player.r.fuel.div(2).plus(1).log2().div(3).plus(1).times(20).div(hasUpgrade("ai", 21)?20:1) }, //HALVEDLOL
		minibotCap() { return player.r.buildings.div(2).floor().max(1).log2().plus(1.5).floor() }, //HALVEDLOL
		buildingEff() { return player.r.buildings.div(2).sub(1).max(0).floor().pow(2).times(50).plus(1) }, //HALVEDLOL
		growTime() { return player.r.allotted.growers.lt(1)?new Decimal(1/0):Decimal.div(15, player.r.allotted.growers.div(2).log10().plus(1)).div(hasUpgrade("ai", 21)?3:1) }, //HALVEDLOL
		producerEff() { //HALVEDLOL
			let mult = hasMilestone("r", 3) ? player.r.grownMinibots.div(8).plus(1) : new Decimal(1); //HALVEDLOL
			if (hasUpgrade("ai", 23)) mult = mult.times(player.r.grownMinibots.times(.2).plus(1)); //HALVEDLOL
			return player.r.allotted.producers.div(2).pow(1.25).div(8).plus(1).times(mult);
		},
		clickables: { //HALVEDLOL
			rows: 2,
			cols: 6,
		...
		milestones: {
			...
			2: { //HALVEDLOL
				...
				effectDescription: "Effective Breeders are ^1.5'D",
			},
			3: { //HALVEDLOL
				requirementDescription: "500 Total Robots",
				done() { return player.r.total.gte(500) },
				effectDescription: "x1.5 Robot gain, and when a Minibot is transformed into a Robot, the requirement for the next Minibot is reduced by 0.25 levels & the Producer effect is 12.5% stronger (additive)",
			},
			4: { //HALVEDLOL
				...
				effectDescription: "Double Robot gain, and the Producer effect also multiplies Watt generation speed & Signal gain",
			},
			5: { //HALVEDLOL
				...
				effectDescription: "Automatically distribute 2.5% of your Robots every second without actually spending any Robots.",
			},
		},
})

addLayer("ai", { //HALVEDLOL
        ...
        gainMult() { // Calculate the multiplier for main currency from bonuses //HALVEDLOL
            mult = new Decimal(1);
			if (hasUpgrade("ai", 22)) mult = mult.times(2); //HALVEDLOL
			if (hasUpgrade("ai", 41)) mult = mult.times(upgradeEffect("ai", 41)); //HALVEDLOL
			if (hasUpgrade("ai", 43)) mult = mult.times(upgradeEffect("ai", 43)); //HALVEDLOL
			if (hasUpgrade("ai", 44)) mult = mult.times(player.ai.buyables[11].div(2).max(1));
            return mult
        },
        gainExp() { // Calculate the exponent on main currency from bonuses
            return new Decimal(1)
		...
		conscEff1() { return player.ai.consc.div(2).plus(1) }, //HALVEDLOL
		conscEff2() { return player.ai.consc.div(2).plus(1).log(3).plus(1) }, //HALVEDLOL
		tabFormat: ["main-display", //HALVEDLOL
			"prestige-button",
			"resource-display", "blank",
			["buyable", 11], "blank",
			...
			["display-text", function() { return "Effect: Multiplies Energy, Signal, & Robot gain by "+format(tmp.ai.conscEff1)+(tmp.nerdMode?" (x/2+1)":"")+", and divides the Robot & Idea req by "+format(tmp.ai.conscEff2)+(tmp.nerdMode?" (log3(x/2+1)+1)":".") }],"blank", "blank",
			["clickable", 11],
		...
		upgrades: {
			...
			11: { //HALVEDLOL
				title: "Node AA",
				description: "Neural Networks are 25% stronger, and unlock a new Neuron milestone.",
				multiRes: [
					{
						cost: new Decimal(2),
					},
			...
			12: { //HALVEDLOL
				title: "Node AB",
				description: "Each Minibot that transforms into a Robot grants you 2.5% of your Robot gain, and start with 2.5 of each Robot specialization.",
				multiRes: [
					{
						cost: new Decimal(10),
					},
			...
			13: { //HALVEDLOL
				title: "Node AC",
				description: "Gear Evolution is 25% stronger, and unlock a new Gear milestone.",
				multiRes: [
					{
			...
			14: { //HALVEDLOL
				title: "Node AD",
				description: "Improve Revelation gain by 25% & improve Gear Evolution power by 5.5%.",
				multiRes: [
					{
						cost: new Decimal(5e3),
					},
			...
			21: { //HALVEDLOL
				title: "Node BA",
				description: "Minibot growth is 3x as fast, and unlock a new Robot milestone, but Miniboth death is 20x as fast.",
				multiRes: [
					{
						cost: new Decimal(15),
					},
			...
			22: { //HALVEDLOL
				title: "Node BB",
				description: "Double Superintelligence gain, start with 2 Ideas, and unlock two new Idea milestones.",
				multiRes: [
					{
						cost: new Decimal(50),
					},
			...
			23: { //HALVEDLOL
				title: "Node BC",
				description: "When a Minibot grows up into a Robot, the requirement for the next Minibot is reduced by 0.25 more levels & the Producer effect is 20% stronger (additive).",
				multiRes: [
					{
			...
			31: { //HALVEDLOL
				title: "Node CA",
				description: "Mech-Energy loss is /1.5'd, and unlock a new Machine milestone.",
				multiRes: [
					{
						cost: new Decimal(300),
					},
			...
			32: { //HALVEDLOL
				title: "Node CB",
				description: "The Idea effect is increased by 0.02, and gain 25% more Revelations.", //HALVEDLOL
				multiRes: [
					{
						cost: new Decimal(500),
					},
			...
			33: { //HALVEDLOL
				title: "Node CC",
				description: "Superintelligence boosts Gear, Machine Part, & Robot gain, & Shell Expansion's boost to Mech-Energy gain is raised ^50.",
				multiRes: [
					{
				...
				unlocked() { return player.ai.unlocked && player.ai.upgrades.length>=4 },
				style: {height: '150px', width: '150px'},
				effect() { return player.ai.points.div(2).plus(1).pow(1.25) },
				effectDisplay() { return format(tmp.ai.upgrades[33].effect)+"x" },
				formula: "(x/2+1)^1.25",
			},
			34: { //HALVEDLOL
				title: "Node CD",
				description: "Super Watts, Mind Watts, & Gear gain are raised ^1.1.",
				multiRes: [
					{
			...
			42: { //HALVEDLOL
				title: "Node DB",
				description: "Each Active AI Node multiplies Signal gain by 50x.",
				multiRes: [
					{
						cost: new Decimal(2e4),
					},
				...
				unlocked() { return player.ai.unlocked && player.ai.upgrades.length>=9 },
				style: {height: '150px', width: '150px'},
				effect() { return Decimal.pow(50, player.ai.upgrades.length) },
...
addLayer("c", { //HALVEDLOL
		...
		update(diff) { //HALVEDLOL
			...
			for (let i=0;i<5;i++) player.c.gainedPower[i] = Decimal.pow(1.5, player.c.gainedPower[i].div(2)).pow(2).plus(Decimal.pow(1.5, player.c.assigned[i].div(2)).sub(1).max(0).times(diff/100)).cbrt().log2();
		},
		power() { //HALVEDLOL
			let data = [];
			for (let i=1;i<=5;i++) data[i] = player.c.points.div(2).sub(i).div(10).plus(1).floor().max(0).sqrt().plus(player.c.gainedPower[i-1]);
			return data;
		},
		...
		eff1() { return tmp.c.power[1].times(12.5) }, //HALVEDLOL
		eff2() { return Decimal.pow(1e10, tmp.c.power[2].div(2)) }, //HALVEDLOL
		eff3() { return Decimal.pow(5e7, tmp.c.power[3].div(2)) }, //HALVEDLOL
		eff4() { return Decimal.pow("1e500", tmp.c.power[4].div(2)) }, //HALVEDLOL
		eff5() { return tmp.c.power[5].div(2).plus(1).log(4).plus(1) }, //HALVEDLOL
		tabFormat: ["main-display",
			"prestige-button",
			"resource-display", "blank",
			["row", [
...
addLayer("a", { //HALVEDLOL
        ...
        achievements: {
			...
			64: {
				name: "Timeless^2",
				done() { return player.h.challenges[31]>=5 },
				tooltip: 'Complete "Timeless" 5 times. Reward: Always keep Row 2 & 3 Upgrades.',
...
addNode("node shenanigans", {
    symbol() {return hasAchievement("ng/", 32)?"S":"???"},
    color: '#791C29',
    layerShown() {return true},
    canClick() {return hasAchievement("ng/", 32)},
	onClick() {return hasAchievement("ng/", 32)?player.tab = "shenanigans":player.tab = player.tab},
    tooltip: "SHENANIGANS",
	tooltipLocked: "???",
}, 
)
...
addNode("node omega", {
	...
	onClick() {player.tab = "omega"},
    tooltip: "OMEGA",
}, 
)

addNode("node paradox", {
    symbol: "P",
    color: 'pink',
    layerShown: true,
    canClick() {return true},
	onClick() {player.tab = "paradox"},
    tooltip: "PARADOX",
}, 
)

addLayer("omega", { //HALVEDLOL
		...
		canBuyMax() { return false },
        row: 999,
        layerShown(){return player.ShowNGLayers},
		automate() {},
		effect() { //HALVEDLOL
			let eff = player.omega.points.div(6)
			return new Decimal(1).add(eff);
		},
		effectDescription() { //HALVEDLOL
			return (player.omega.points>1?"which are":"which is")+" boosting Point generation by "+format(tmp.omega.effect)+"^ after everything (applies at >1 points per second)"+(tmp.nerdMode?"\n (+^1.16 each)":"")
		},
		milestones: {
			0: {
				requirementDescription: "1st Omega Level",
				done() { return  player.omega.points >= 1 },
				effectDescription: "Back to the roots:<br>Multiplies point gain by 2.",
			},
			1: {
				requirementDescription: "2nd Omega Level",
				done() { return  player.omega.points >= 2 },
				effect() {
					let b = Decimal.add(player.b.upgrades.length, 1);
					let g = Decimal.add(player.g.upgrades.length, 1);
					return {
						b: g.log10().add(1).root(3),
						g: b.log10().add(1).root(2),
					}
				},
				unlocked() { return  player.omega.points >= 1 },
				effectDescription() { return "Dual Synchronization:<br>Booster and Generator upgrades are boosting their counterpart layer's effect.<br>Booster: x"+format(tmp.omega.milestones[1].effect.b)+" "+(tmp.nerdMode?"\n (cbrt(log10(n)+1))":"")+"<br>Generator: x"+format(tmp.omega.milestones[1].effect.g)+" "+(tmp.nerdMode?"\n (sqrt(log10(n)+1))":"")},
			},
			2: {
				requirementDescription: "3rd Omega Level",
				done() { return player.omega.points >= 3 },
				effectDescription: "I hate Space layer:<br>Omega Levels affect Row 3 layers.<br>Quinary Space building gets 125 free levels.",
				unlocked() { return  player.omega.points >= 2 },
			},
			3: {
		...
		doReset(resettingLayer) {
			...
			player.g.unlockOrder = 0
			player.g.unlocked = false
			player.s.unlockOrder = 0
			player.s.unlocked = false
			player.e.unlockOrder = 0
			player.e.unlocked = false
			player.t.unlockOrder = 0
			player.t.unlocked = false
			player.o.unlockOrder = 0
			player.o.unlocked = false
			player.ss.unlockOrder = 0
			player.ss.unlocked = false
			player.en.unlockOrder = 0
			player.en.unlocked = false
			player.ne.unlockOrder = 0
			player.ne.unlocked = false
			player.hs.unlockOrder = 0
			player.hs.unlocked = false
			player.n.unlockOrder = 0
			player.n.unlocked = false
			player.sb.unlocked = false
			player.sg.unlocked = false
			player.h.unlocked = false
			player.q.unlocked = false
			player.m.unlocked = false
			player.ba.unlocked = false
			player.ps.unlocked = false
			player.hn.unlocked = false
			player.ma.unlocked = false
			player.i.unlocked = false
			player.ge.unlocked = false
			player.mc.unlocked = false
			player.id.unlocked = false
...
addLayer("paradox", { //HALVEDLOL
		startData() { return {
			unlocked: true,
			points: new Decimal(0),
			storedPoints: new Decimal(0),
			storedPrestigePoints: new Decimal(0),
        ...
        position: 3, // Horizontal position within a row. By default it uses the layer id and sorts in alphabetical order
        color: "pink",
        requires() { return new Decimal(60000) }, // Can be a function that takes requirement increases into account
        resource() {return player.paradox.points.gt(1)?"paradoxes":"paradox"}, // Name of prestige currency
        baseResource: "points", // Name of resource prestige is based on
        baseAmount() {return player.points}, // Get the current amount of baseResource
        type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
        exponent() { return 0.0054513 }, // Prestige currency exponent
		gainMult() { 
			let mult = new Decimal(1);
			return mult;
		},
		canBuyMax() { return false },
        row: 999,
        layerShown(){return player.ShowNGLayers && hasAchievement("ng/", 13)},
		automate() {},
		effect() { //HALVEDLOL
			return Decimal.pow(player.paradox.points.add(1).max(0), 2).log(3).add(1).pow(2)
		},
		effect2() {
			return player.paradox.paradoxDead.pow(3).root(9).add(1).mul(tmp.s.effect).add(1)
		},
		effectPow() {
			return player.paradox.paradoxPower.pow(3).root(9).add(1)
		},
		effectDescription() { //HALVEDLOL
			return "which boosts global speed by "+format(tmp.paradox.effect.root(2))+"x "+(tmp.nerdMode?"(log3((n+1)^2)+1)":"")
		},
		update(diff) {
			if(player.points.gte(Decimal.pow(10, 9048)) && !player.shenanigans.unlocked) player.shenanigans.unlocked = true
			let Diviser = new Decimal(2.5)
			let Limiter = Decimal.dNumberMax.log10()
			if (hasUpgrade("paradox", 14)) Limiter = Limiter.mul(tmp.t.enEff)
			if (hasUpgrade("paradox", 11)) Diviser = Diviser.add(1)
			if (hasUpgrade("paradox", 12)) Diviser = Diviser.add(0.5)
			let theThing = player.paradox.paradoxPower.mul(diff).div(Diviser)
			player.paradox.paradoxDead = player.paradox.paradoxDead.add(theThing)
			if(tmp.paradox.effectPow.gte(Limiter)) Diviser = Diviser.log10().add(1).div(tmp.paradox.effectPow.log(Limiter).pow(3))
			theThing = player.paradox.paradoxPower.mul(diff).div(Diviser)
		...
		buyables: { //HALVEDLOL
			...
			11: {
				...
				display() { // Everything else displayed in the buyable button after the title
                    return hasAchievement("ng/", 13)?"You currently have "+format(player.paradox.storedPoints)+" points stored.":""
                },
			...
			12: {
				title() { return hasAchievement("ng/", 13)?"Store 50% of prestige points":"Do Paradox Reset to unlock" },
				effect() { return new Decimal(0) },
				display() { // Everything else displayed in the buyable button after the title
                    return hasAchievement("ng/", 13)?"You currently have "+format(player.paradox.storedPrestigePoints)+" prestige points stored.":""
                },
                unlocked() { return true }, 
                canAfford() {
					return hasAchievement("ng/", 13) && hasUpgrade("p", 11)
                ...
                buy() { 
					let halfPoints = player.p.points.div(2)
			...
			31: {
				title() { return hasAchievement("ng/", 13)?"<h3>Annihilate stored points and prestige points.<h3/>":"Do Paradox Reset to unlock" },
				effect() { return new Decimal(0) },
				display() { // Everything else displayed in the buyable button after the title
                    return hasAchievement("ng/", 13)?"<h4>You will gain "+format(tmp.paradox.buyables[31].gain)+" Paradoxical Prestigious Particles "+(tmp.nerdMode?"(yroot100(p^14xpp^16.5)-1)":"<h4/>"):"<h4/>"
                },
                unlocked() { return true }, 
                canAfford() {
					return hasAchievement("ng/", 13)
				...
				gain() {
					let gain = player.paradox.storedPoints.pow(14).mul(player.paradox.storedPrestigePoints.pow(16.5)).root(100).sub(1).max(0)
					if (hasAchievement("ng/", 25)) gain = gain.times(1e42)
					return gain
				},
                buy() { 
					player.paradox.paradoxPower = player.paradox.paradoxPower.add(tmp.paradox.buyables[31].gain)
					player.paradox.didyagetit = tmp.paradox.buyables[31].gain
		...
		upgrades: {
			...
			cols: 6,
				11: {
					title: "Anti-Decay",
					description: "PPP's Decaying Factor is less harsh.",
					cost() { return new Decimal(10) },
					unlocked() { return hasAchievement("ng/", 13) },
				...
				12: {
					title: "Spare Change",
					description: "PPP's Decaying Factor is slighty less harsh.",
				...
				13: {
					title: "Paradox Space",
					description: "Space energies are disturbing DDD, granting them PPP's effect partially.",
					cost() { return new Decimal("3e61") },
					unlocked() { return hasAchievement("ng/", 13) && player.s.unlocked },
					currencyInternalName: "paradoxDead",
					currencyDisplayName: "Dissolved Drained Distortions",
					currencyLayer: "paradox",
				},
				14: {
					title: "Time Breaker",
					description: "Time Capsules affect PPP's limiter and PPP's limiter's boosts point gain.",
					effect() { return new Decimal(308.25471555991675).mul(tmp.t.enEff) },
					effectDisplay() { return format(this.effect())+"x" },
					formula() { return "308.25*x" },
					cost() { return new Decimal("1e63") },
					unlocked() { return hasAchievement("ng/", 13) && player.t.unlocked },
					currencyInternalName: "paradoxDead",
					currencyDisplayName: "Dissolved Drained Distortions",
					currencyLayer: "paradox",
				},
				15: {
					title: "Boosted Enhancers",
					description: "Enhancers are boosting Booster's base.",
				...
				21: {
					title: "SUPER PRESTIGE = BOOSTER?!",
					description: "Boosts Super Booster's base by the amount of layers you've unlocked.",
					effect() { return unlockedLayers().div(2).add(1) },  
					effectDisplay() { return "x"+format(this.effect()) },
					formula() { return "x/2+1" },
					cost() { return new Decimal("1e2226") },
					unlocked() { return hasAchievement("ng/", 32) },
					currencyInternalName: "paradoxDead",
					currencyDisplayName: "Dissolved Drained Distortions",
					currencyLayer: "paradox",
				},
				22: {
					title: "The Root Of All Points",
					description: "Booster's effect boost Generator Power's generation.",
...
addLayer("ng/", { //HALVEDLOL
		...
		tabFormat: {
			"Main Hub": {
				content: [
					"blank", "blank",
					["display-text", function() { return "<h1 style='color: #c20000;'>Welcome Back.<h1/><br><br><h3 style='color: #b20000;'>Come have a seat as you'll visit this tab often.<h3/><br><br><br><br>"}], ["tree", testTree], ["display-text", function() { return "<br><h5 style='color: #520000;'>we're not joking btw you will spend most of your time coming here<h5/><br><br><br><br><h3>Note: NG/ layers reset the ENTIRE game (It's worth it)<br><br>You have been warned.<h3/>" }], 
					"blank", "blank", 
				],
			},
			"Achievements": {
		...
		achievements: {
            ...
            cols: 6,
			11: {
                name: "Did you know TPT NG+ exists? No?...<br><br>ooooh my bad",
                done() { return player.omega.points >= 1 },
				onComplete() { player["ng/"].thoseAchievements = player["ng/"].thoseAchievements.add(1) },
                tooltip: "Reach 1st Omega Level.",
				image: "images/achs2/11.png",
            },
            12: {
                name: "Hog Challenger",
                done() { return getPointGen() >= 25 },
				onComplete() { player["ng/"].thoseAchievements = player["ng/"].thoseAchievements.add(1) },
                tooltip: "Reach 25 points per second.",
				image: "images/achs2/12.png",
            },
            13: {
                name: "So many nerfs to deal with, so little time. Wouldn't you agree?",
				onComplete() { player["ng/"].thoseAchievements = player["ng/"].thoseAchievements.add(1) },
                done() { return player.paradox.points >= 1 },
                tooltip: "Unlock Paradox layer.",
				image: "images/achs2/13.png",
            },
            14: {
                name: "I should've chosen Generator instead",
                done() { return player.b.points.gte(4) },
				onComplete() { player["ng/"].thoseAchievements = player["ng/"].thoseAchievements.add(1) },
                tooltip: "Reach 4 Boosters.",
				image: "images/achs2/14.png",
            },
            15: {
            ...
            16: {
                name: "Obligatory Ocean ref... Wait, what?",
                done() { return player.g.power.gte(1006) },
				onComplete() { player["ng/"].thoseAchievements = player["ng/"].thoseAchievements.add(1) },
                tooltip: "Reach 1006 Generator Power.",
				image: "images/achs2/16.png",
            },
			21: {
                name: "Again?",
                done() { return player.omega.points >= 2 },
				onComplete() { player["ng/"].thoseAchievements = player["ng/"].thoseAchievements.add(1) },
                tooltip: "Reach 2nd Omega Level.",
				image: "images/achs2/21.png",
            },
            22: {
                name: "2048 is love. 2048 is life.",
				onComplete() { player["ng/"].thoseAchievements = player["ng/"].thoseAchievements.add(1) },
                done() { return player.paradox.didyagetit.gte(2048) },
                tooltip: "Gain 2048 Paradoxical Prestigious Particles in a single annihilation.",
				image: "images/achs2/22.png",
            },
            23: {
                name: "Dead. All of them",
				onComplete() { player["ng/"].thoseAchievements = player["ng/"].thoseAchievements.add(1) },
                done() { return player.paradox.paradoxDead.gte("1.08e11") },
                tooltip: "Reach 108 billion Dissolved Drained Distortions.",
				image: "images/achs2/23.png",
            },
            24: {
                name: "Dual Disaster",
				onComplete() { player["ng/"].thoseAchievements = player["ng/"].thoseAchievements.add(1) },
			...
			31: {
				...
				onComplete() { player["ng/"].thoseAchievements = player["ng/"].thoseAchievements.add(1) },
                tooltip: "Reach 3rd Omega Level.",
				image: "images/achs2/31.png",
            },
			32: {
			...
			41: {
                name: "You did it. You crazy son of a bitch, you did it!",
                done() { return player.omega.points >= 4 },
				onComplete() { player["ng/"].thoseAchievements = player["ng/"].thoseAchievements.add(1) },
                tooltip: "Reach 4th Omega Level.",
				image: "images/achs2/41.png",
            },
			42: {
...
addLayer("shenanigans", { //HALVEDLOL
        startData() { return {
            unlocked: false,
		...
		position: 0,
		symbol: "₴",
        layerShown() {return player.ShowNGLayers && hasAchievement("ng/", 32)}, 
        tooltip() { // Optional, tooltip displays when the layer is locked
		...
		infoboxes: {
			lore: {
				title: "foo, the Spirit of All Shenanigans",
				body() { return "Rise and shine, Mister Freeman. Rise and... shine. Not that I... wish to imply you have been sleeping on the job. No one is more deserving of a rest... and all the effort in the world would have gone to waste until... well, let's just say your hour has... come again.<br><br>The right man in the wrong place can make all the difference in the world. So, wake up, Mister Freeman. Wake up and...<br><br><br><br>... Huh? Oh, my bad. I didn't notice you as I was monolouging that funny looking copypasta. Haven't expected anyone to go past Row 2... That is, unless, you have power to overcome stupidous obstacles. Timewalls or not.<br><br>I'm assuming you got stuck mid-way through Row 3, am I correct? Well, since you've come this far, I shall bestow you with my power. Power so cool that it doesn't destroy anything. Really.<br><br>I was never a fan of Omega or Paradox as they tend to be repetitive and... an annoyance to say the least. Have fun." },
				bodyStyle() { return 'color: #791C29' },
		...
		buyables: { //HALVEDLOL
            ...
            respec() { // Optional, reset things and give back your currency. Having this function makes a respec button appear
				player[this.layer].ShenanigansChaosAntiBalancers = player[this.layer].ShenanigansChaosAntiBalancers.add(player[this.layer].refund)
                resetBuyables(this.layer)
				player[this.layer].refund = new Decimal(0);
            },
            respecText: "Refund Shenanigans Buildings (Doesn't force reset.)", // Text on Respec button, optional
			11: {
				cost() { return new Decimal(6).pow(player.shenanigans.buyables[11]) },
				title() { return "Scrumptiously Charismatic Accelerating Boosters" },
				effect() { return new Decimal(60).pow(player.shenanigans.buyables[11]) },
				display() { // Everything else displayed in the buyable button after the title
                    return (tmp.nerdMode?("Cost Formula: 6^x"):("Cost: "+format(tmp.shenanigans.buyables[11].cost)+" "+(tmp.shenanigans.buyables[11].cost.gt(1)?"SCABs":"SCAB")))+"<br>Amount: "+formatWhole(player.shenanigans.buyables[11])+"<br>"+(tmp.nerdMode?("Formula: 60^x"):("Boosts your point and prestige point gain by x"+format(tmp.shenanigans.buyables[11].effect)))
                },
                unlocked() { return player.shenanigans.unlocked }, 
			...
			21: {
				...
				title() { return "Sacred Cinematic Amplifiers (Beta)" },
				effect() { return new Decimal(1.5).pow(new Decimal(player.shenanigans.buyables[21]).times(tmp.shenanigans.buyables[31].effect)) },
				display() { // Everything else displayed in the buyable button after the title
                    return (tmp.nerdMode?("Cost Formula: 400*(12^x)"):("Cost: "+format(tmp.shenanigans.buyables[21].cost)+" "+(tmp.shenanigans.buyables[21].cost.gt(1)?"SCABs":"SCAB")))+"<br>Amount: "+formatWhole(new Decimal(player.shenanigans.buyables[21]).times(tmp.shenanigans.buyables[31].effect))+"<br>"+(tmp.nerdMode?("Formula: 1.5^x"):("Boosts Booster's base by x"+format(tmp.shenanigans.buyables[21].effect)))
                },
                unlocked() { return player.shenanigans.unlocked }, 
			...
			81: {
				...
				title() { return "Shenanigious Chaotic Anti Balancer" },
				effect() { return new Decimal(60).pow(player.shenanigans.buyables[81]) },
				display() { // Everything else displayed in the buyable button after the title
                    return (tmp.nerdMode?("Cost Formula: 10^((x)*(x))"):("Cost: "+format(tmp.shenanigans.buyables[81].cost)+" "+(tmp.shenanigans.buyables[81].cost.gt(1)?"SCABs":"SCAB")))+"<br>Amount: "+formatWhole(player.shenanigans.buyables[81])+"<br>"+(tmp.nerdMode?("Formula: 60^x"):("Boosts SCABs gain by x"+format(tmp.shenanigans.buyables[81].effect)))
                },
